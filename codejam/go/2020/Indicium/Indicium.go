...
func main() {
    ...
    for tt:=1;tt<=T;tt++ {
		...
		if K == N+1 || K == N*N-1 || (N == 3 && (K==5 || K==7)) {
			...
			square = make([][]int,N); for i:=0;i<N;i++ { square[i] = make([]int,N) }
			square[0][0] = B; square[1][1] = C; for i:=2;i<N;i++ { square[i][i] = A }
			for r:=0;r<N;r++ {
			...
			if N == 2 && K == 2 { fmt.Fprintf(wrtr,"Case #%v: %v\n1 2\n2 1\n",tt,"POSSIBLE") }
			if N == 2 && K == 4 { fmt.Fprintf(wrtr,"Case #%v: %v\n2 1\n1 2\n",tt,"POSSIBLE") }
			if N == 3 && K == 3 { fmt.Fprintf(wrtr,"Case #%v: %v\n1 2 3\n3 1 2\n2 3 1\n",tt,"POSSIBLE") }
			if N == 3 && K == 6 { fmt.Fprintf(wrtr,"Case #%v: %v\n1 3 2\n3 2 1\n2 1 3\n",tt,"POSSIBLE") } 
			else if N == 3 && K == 9 {
			fmt.Fprintf(wrtr,"Case #%v: %v\n3 2 1\n1 3 2\n2 1 3\n",tt,"POSSIBLE")
		} else { 
			diag := make([]int,N)
			if K % N == 0 {
				for i:=0; i<N;i++ { diag[i] = K/N }
			} else {
				for i:=1; i<=N; i++ {
					lb := (N-2)*i + 2; ub := (N_2)*i + 2*N
					if K < lb || K > ub { continue }
