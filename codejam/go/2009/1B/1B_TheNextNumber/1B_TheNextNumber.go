...
func next_byte_permutation(a []byte) bool {
...
func main() {
    ...
    T := gi()
    for tt:=1;tt<=T;tt++ {
		s := gbs(); s2 := make([]byte,0,len(s)+1); s2 = append(s2,'0'); for _,c := range s { s2 = append(s2,c) }
		next_byte_permutation(s2)
		var ans string
		if s2[0] == '0' { ans = string(s2[1:]) } else { ans = string(s2[0:]) }
