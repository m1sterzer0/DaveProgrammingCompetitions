...
func main() {
	...
	T := gi()
    for tt:=1;tt<=T;tt++ {
		H,W := gi(),gi(); gr := make([][]int,H); for i:=0;i<H;i++ { gr[i] = gis(W) }
		du := NewDsu(H*W)
		for i:=0;i<H;i++ {
			for j:=0;j<W;j++ {
				best := gr[i][j]; ii := -1; jj := -1
				if i-1 >= 0 && gr[i-1][j] < best { best,ii,jj = gr[i-1][j],i-1,j }
				if j-1 >= 0 && gr[i][j-1] < best { best,ii,jj = gr[i][j-1],i,j-1 }
				if j+1 < W  && gr[i][j+1] < best { best,ii,jj = gr[i][j+1],i,j+1 }
				if i+1 < H  && gr[i+1][j] < best { best,ii,jj = gr[i+1][j],i+1,j }
				if ii != -1 { du.Merge(W*ii+jj,W*i+j) }
			}
		}
        fmt.Fprintf(wrtr,"Case #%v:\n",tt)
		lookup := iai(H*W,-1); curs := 0
		for i:=0;i<H;i++ {
			ansarr := make([]byte,2*W-1); for j:=0;j<2*W-1;j++ { ansarr[j] = ' '; }
			for j:=0;j<W;j++ {
				l := du.Leader(i*W+j);
				if lookup[l] == -1 {lookup[l] = curs; curs++ } 
				ansarr[2*j] = byte('a'+lookup[l])
			}
