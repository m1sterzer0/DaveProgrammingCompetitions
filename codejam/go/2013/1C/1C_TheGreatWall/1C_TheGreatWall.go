...
func main() {
    ...
    T := gi()
    for tt:=1;tt<=T;tt++ {
		// lazy seg tree with coordinate compression??
		N := gi()
		D,NN,W,E,S,dd,dp,ds := ia(N),ia(N),ia(N),ia(N),ia(N),ia(N),ia(N),ia(N)
		for i:=0;i<N;i++ { D[i],NN[i],W[i],E[i],S[i],dd[i],dp[i],ds[i] = gi(),gi(),gi(),gi(),gi(),gi(),gi(),gi() }
		// Do the coordinate compression first
		rawcoords := make([]int,0,2000*N)
		for i:=0;i<N;i++ { for j:=0;j<NN[i];j++ { rawcoords = append(rawcoords,W[i]+dp[i]*j,E[i]+dp[i]*j) } }
		uc := sortUniq(rawcoords)
		// Need to deal with the space between the coordinates, so double things
		cmap := make(map[int]int); for i,c := range uc { cmap[c] = 2*i }
		// Set up the lazy seg tree
		lstop := func(a,b int) int   { if a < b { return a }; return b }
		lstcomp := func(f,g int) int { if f > g { return f};  return g }
		lstmap := func(f,a int) int  { if f > a { return f }; return a }
		v := iai(2*len(uc),0); lst := NewlazysegtreeVec(v,lstop,lstmap,lstcomp,1<<60,0)

		// Create the list of events
		type event struct { d,w,e,s int }
		ee := make([]event,0,2000*N)
		for i:=0;i<N;i++ { 
			for j:=0;j<NN[i];j++ {
				w := cmap[W[i]+dp[i]*j]
				e := cmap[E[i]+dp[i]*j]-1
				d := D[i] + j*dd[i]
				s := S[i] + j*ds[i]
				ee = append(ee,event{d,w,e,s})
			}
		}

		// Sort and process the events
		sort.Slice(ee,func(i,j int) bool { return ee[i].d < ee[j].d } )
		ans := 0; idx := 0; nevents := len(ee)
		for idx < nevents {
			d := ee[idx].d
			jidx := idx; for jidx+1 < nevents && ee[jidx+1].d == d { jidx++ }
			for i:=idx;i<=jidx;i++ {
				ss := lst.Prod(ee[i].w,ee[i].e)
				if ss < ee[i].s  { ans++ }
			}
			for i:=idx;i<=jidx;i++ { 
				lst.ApplyRange(ee[i].w,ee[i].e,ee[i].s)
			}
			idx = jidx+1
		}
        fmt.Printf("Case #%v: %v\n",tt,ans)
