...
func main() {
    ...
    T := gi()
    for tt:=1;tt<=T;tt++ {
		N := gi(); X,Y,Z,P := make([]float64,N),make([]float64,N),make([]float64,N),make([]float64,N)
		for i:=0;i<N;i++ { X[i],Y[i],Z[i],P[i] = gf(),gf(),gf(),gf() }
		tryit := func(m float64) bool {
			inf := 1e99
			a1,a2,a3,a4,b1,b2,b3,b4 := -inf,-inf,-inf,-inf,inf,inf,inf,inf
			for i:=0;i<N;i++ {
				x,y,z,p := X[i],Y[i],Z[i],P[i]
				d := m * p
				a1 = maxf(a1,+x+y-z-d)
				a2 = maxf(a2,+x-y+z-d)
				a3 = maxf(a3,-x+y+z-d)
			...
			l,r := a1+a2+a3,b1+b2+b3
			if l > b4 || r < a4 { return false }
			return true
		}
		l,u := 0.0,3.6e6
		for i:=0;i<60;i++ {
