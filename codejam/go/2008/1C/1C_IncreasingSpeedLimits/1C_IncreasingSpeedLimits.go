...
func main() {
    ...
    for tt:=1;tt<=T;tt++ {
		...
		// Coordinate compression first
		S2 := ia(n); copy(S2,S); S2 = sortUniq(S2)
		lkup := make(map[int]int)
		for i,s := range S2 { lkup[s] = i }
		S3 := ia(n); for i:=0;i<n;i++ { S3[i] = lkup[S[i]] }

		// Use a segment tree
		lstop := func(a,b int) int { return (a+b) % MOD }
		st := Newsegtree(len(S2),lstop,0)
		ans := 0
		for _,s := range S3 {
			lans := 1
