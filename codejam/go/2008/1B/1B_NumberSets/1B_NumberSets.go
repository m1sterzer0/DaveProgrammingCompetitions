...
func sieve(N int) []int {
	sb := make([]bool,N+1); for i:=0;i<=N;i++ { sb[i] = true }; sb[0] = false; sb[1] = false
	for i:=4;i<=N;i+=2 { sb[i] = false }
	for i:=3;i<=N;i+=2 {
		if sb[i] {
			for j:=i*i;j<=N;j+=2*i { sb[j] = false }
		}
	}
	ans := make([]int,0)
	for i:=0;i<=N;i++ { if sb[i] { ans = append(ans,i) } }
	return ans
}
...
func main() {
    ...
    for tt:=1;tt<=T;tt++ {
		...
		for _,p := range primes {
			if p < P { continue }
			if p > B-A { break }
			start := 0; if A%p != 0 { start = p - A%p }
			for i:=start; i<=B-A; i+=p { 
				if i+p <= B-A { du.Merge(i,i+p) }
			}
		}
		ans := 0
		for i:=0;i<=B-A;i++ { if i == du.Leader(i) { ans++ } }
