...
func main() {
    ...
    for tt:=1;tt<=T;tt++ {
		...
		adjew := func(i,j int) bool {
			if X2[i]+1 != X1[j] && X2[j]+1 != X1[i] { return false }
			if Y2[i] < Y1[j] || Y2[j] < Y1[i] { return false }
			return true
		}
		adjns := func(i,j int) bool {
			if Y2[i]+1 != Y1[j] && Y2[j]+1 != Y1[i] { return false }
			if X2[i] < X1[j] || X2[j] < X1[i] { return false }
			return true
		}
		adjcorner := func(i,j int) bool {
			if X2[i]+1 == X1[j] && Y1[i]-1 == Y2[j] { return true }
			if X2[j]+1 == X1[i] && Y1[j]-1 == Y2[i] { return true }
			return false
		}
		uf := NewDsu(N)
		for i:=0;i<N;i++ {
			for j:=i+1;j<N;j++ {
				if overlap(i,j) || adjew(i,j) || adjns(i,j) || adjcorner(i,j) { uf.Merge(i,j) }
			}
		}
		ans := 0
		x := uf.Groups()
		for _,g := range x {
			xmax,ymax,minxpy := -1,-1,1<<60
			for _,i := range g {
				xmax = max(xmax,X2[i])
				ymax = max(ymax,Y2[i])
				minxpy = min(minxpy,X1[i]+Y1[i])
			}
