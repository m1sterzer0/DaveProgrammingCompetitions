...
struct bigint {
	...
	bigint operator ^(const bigint &v){
		...
		while(!b.isZero()){ if(b%2) ans*=a; a*=a,b/=2; }
		return ans;
	}
	...
	bigint() : sign(1) {}
 	...
 	void operator=(long long v) { sign = 1; a.clear(); if (v<0) sign = -1, v = -v; while(v>0) { a.push_back(v % base); v/=base; } }
	...
	bigint operator*(int v) const { bigint res = *this; res *= v; return res; }
 	void operator*=(long long v) {
		if (v < 0) sign = -sign, v = -v;
		if(v > base) { *this = *this * (v / base) * base + *this * (v % base); return; }
		for (int i = 0, carry = 0; i < (int) a.size() || carry; ++i) {
			if (i == (int) a.size()) a.push_back(0);
			long long cur = a[i] * (long long) v + carry;
			carry = (int) (cur / base);
	...
	friend bigint lcm(const bigint &a, const bigint &b) { return a / gcd(a, b) * b; }
 	void read(const string &s) {
		...
		for (int i = s.size() - 1; i >= pos; i -= base_digits) {
			int x = 0; for (int j = max(pos, i - base_digits + 1); j <= i; j++) x = x * 10 + s[j] - '0'; a.push_back(x);
		}
		trim();
	}
...
int main (int argc, char **argv) {
    ...
    for (ll tt=1;tt<=T;tt++) {
        ll N; cin >> N;
        vector<bigint> TT(N); FOR(i,N) cin >> TT[i];
