...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		ft2 := NewFenwick(N+5)
		ft3 := NewFenwick(N+5)
		cnt := ia(4); farr := []*Fenwick{ft1,ft1,ft2,ft3}; 
		for i,a := range A { cnt[a]++; farr[a].Inc(i+1,1) }
		ans := 0
		solve := func(b1,b2,b3,c1,c2,c3 int) int {
			d1,d2,d3 := c1-b1,c2-b2,c3-b3
			tot1 := b1+2*b2+3*b3
			tot2 := d1+2*d2+3*d3
			if tot1 > tot2 { tot1,tot2,b1,b2,b3,d1,d2,d3 = tot2,tot1,d1,d2,d3,b1,b2,b3 }
			if (tot1+tot2) % 2 == 1 { return -1 }
			if tot1==tot2 { return 0 }
			swaps := 0
			swap31 := min(min((tot2-tot1)/4,d3),b1)
			swaps += swap31; d3 -= swap31; d1 += swap31; b3 += swap31; b1 -= swap31; tot2 -= 2*swap31; tot1 += 2*swap31
			swap32 := min(min((tot2-tot1)/2,d3),b2)
			swaps += swap32; d3 -= swap32; d2 += swap32; b3 += swap32; b2 -= swap32; tot2 -= swap32; tot1 += swap32
			swap21 := min(min((tot2-tot1)/2,d2),b1)
			swaps += swap21; d2 -= swap21; d1 += swap21; b2 += swap21; b1 -= swap21; tot2 -= swap21; tot1 += swap21
		...
		for i:=0;i<M;i++ {
			...
			farr[y].Inc(x+1,1)
			cnt[A[x]]--; cnt[y]++; A[x] = y
			swaps := solve(ft1.Prefixsum(z+1),ft2.Prefixsum(z+1),ft3.Prefixsum(z+1),cnt[1],cnt[2],cnt[3])
			ans += swaps
		}
		fmt.Printf("Case #%v: %v\n",tt,ans)
	}
