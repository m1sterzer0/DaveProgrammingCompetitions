...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		uf := NewDsu(N)
		carr := ia(N); for i:=0;i<N;i++ { carr[i] = i }
		ans := -1
		curs := 0
		for curs < N && uf.Same(curs,curs/K*K) { curs++ }
		if curs == N { ans = 0 }
		for i:=0;i<M;i++ {
			a,b := A[i],B[i]
			if carr[a] == -1 { 
				...
				carr[b] = carr[a]; carr[a] = -1; continue
			} else {
				uf.Merge(carr[a],carr[b]); carr[a] = -1
				for curs < N && uf.Same(curs,curs/K*K) { curs++ }
				if curs == N { ans = i+1; break }
			}
		} 
		fmt.Printf("Case #%v: %v\n",tt,ans)
	}
