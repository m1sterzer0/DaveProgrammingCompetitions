...
func main() {
	...
	T := gi()
	// Need to find n s.t. -S + nK = 0 (mod N) --> nK = S (mod N)
	// First need to calculate gcd of K an N.  If not 1 and it doesn't divide (-S), we are done.  Otherwise, we divide everything by the GCD.
	// Now we need to calculate the modular inverse of K with extended euclidean algorithm 
	for t:=1;t<=T;t++ {
		N,S,K := gi3()
		S = N-S
		g := gcd(N,K)
		if g != 1 {
			if S % g != 0 { fmt.Fprintln(wrtr,-1); continue }
			N /= g; K /= g; S /= g
		}
		kinv,_ := modinv(K,N)
		fmt.Fprintln(wrtr,kinv*S % N )
	}
}
