...
func (q *MinCostFlow) AddEdge(fr, to, cap, cost int) {
	//fmt.Printf("DBG: fr,to,cap,cost = %v,%v,%v,%v\n",fr,to,cap,cost);
...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); A,B,C := fill3(N); for i:=0;i<N;i++ { A[i]--; B[i]-- }
	// Nodes
	// 0 is source, 1 is sink, 2 is governor, 3 through 152 universities, 153 to 302 are subjects
	cmax := 1000000000
	mcf := NewMinCostFlow(310)
	for i:=0;i<150;i++ { mcf.AddEdge(2,3+i,1,0) }
	for i:=0;i<150;i++ { mcf.AddEdge(153+i,1,1,0) }
	for i:=0;i<N;i++ { a,b,c := A[i],B[i],C[i];  mcf.AddEdge(3+a,153+b,1,cmax-c) }
	cumcost := 0
	ansarr := make([]int,0)
	for i:=1;i<=N;i++ {
		mcf.AddEdge(0,2,1,0)
