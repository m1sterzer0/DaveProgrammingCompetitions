...
func main() {
	...
	dp,ndp := make([]bool,X+1),make([]bool,X+1)
	dp[0] = true
	dd := Newdeque()
	for ii,a := range A {
		b := B[ii]
		for r:=0;r<a;r++ {
			dd.Clear()
			for i:=r;i<=X;i+=a {
				for (dd.Len() > 0) && (dd.Head() < i - a*b) { dd.PopFront() }
				if dp[i] { dd.PushBack(i) }
				if dd.Len() > 0 { ndp[i] = true } else { ndp[i] = false }
			}
		}
		dp,ndp = ndp,dp
	}
	ans := "No"; if dp[X] { ans = "Yes" }; fmt.Println(ans)
}
