...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); L,R := fill2(N)
		balls := make([]ball,N); for i:=0;i<N;i++ { balls[i] = ball{L[i],R[i]} }
		sort.Slice(balls,func(i,j int) bool { return balls[i].l < balls[j].l } )
		good := true; eptr := 0; t := 0
		mh := Newminheap(func(a,b int) bool { return a < b })
		for eptr < N || !mh.IsEmpty() {
			for eptr < N && balls[eptr].l == t { mh.Push(balls[eptr].r); eptr++ }
			if !mh.IsEmpty() { xx := mh.Pop(); if xx < t { good = false; break } }
			if mh.IsEmpty() && eptr < N { t = balls[eptr].l } else { t++ }
		}
		if good { fmt.Fprintln(wrtr,"Yes") } else { fmt.Fprintln(wrtr,"No") }
	}
}
