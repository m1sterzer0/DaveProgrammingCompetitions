...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M := gi(),gi(); A,B := fill2(M); C := gis(N); for i:=0;i<M;i++ { A[i]--; B[i]-- }
	// Nodes 0 to N-1 are node inputs
	// Nodes N to 2N-1 are node outputs
	flow := NewMfgraph(2*N)
	for i:=1;i<N-1;i++ { flow.Addedge(i,N+i,C[i]) }
	for i:=0;i<M;i++ { a,b := A[i],B[i]; flow.Addedge(N+a,b,inf); flow.Addedge(N+b,a,inf) }
	f := flow.Flow(N,N-1)
	ansarr := make([]int,0)
	mc := flow.Mincut(N)
	for i:=1;i<N-1;i++ { if mc[i] && !mc[N+i] { ansarr = append(ansarr,i+1) } }
