...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M,K := gi(),gi(),gi(); U,V,A := fill3(M); for i:=0;i<M;i++ { U[i]--; V[i]-- }; S := gis(K); for i:=0;i<K;i++ { S[i]-- }
	type pair struct { n,d int }
	gr := make([][]pair,2*N)
	for i:=0;i<M;i++ {
		u,v := U[i],V[i]; if A[i] == 0 { u += N; v += N }
		gr[u] = append(gr[u], pair{v,1}); gr[v] = append(gr[v], pair{u,1})
	}
	for _,s := range S { gr[s] = append(gr[s],pair{s+N,0}); gr[s+N] = append(gr[s+N],pair{s,0}) }
	dd := iai(2*N,-1); dd[0] = 0; q := Newdeque(); q.PushBack(0)
	for !q.IsEmpty() {
		n := q.PopFront()
		for _,e := range gr[n] {
			if dd[e.n] != -1 { continue }
			if e.d == 0 { dd[e.n] = dd[n]; q.PushFront(e.n) }
			if e.d == 1 { dd[e.n] = dd[n]+1; q.PushBack(e.n) }
		}
	}
	ans := 1<<60
	if dd[N-1]==-1 && dd[2*N-1] == -1 { ans = -1 }
	if dd[N-1] != -1 { ans = min(ans,dd[N-1]) }
	if dd[2*N-1] != -1 { ans = min(ans,dd[2*N-1]) }
	fmt.Println(ans)
}
