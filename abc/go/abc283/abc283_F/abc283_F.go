...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); P := gis(N);
	// Clearly we need a NlogN or at worse, NsqrtN solution
	// The abs value is problematic
	inf := 1<<60
	D := iai(N+1,inf)

	type pair struct {i,pi int }
	PP := make([]pair,0)
	for i:=0;i<N;i++ { PP = append(PP,pair{i+1,P[i]}) }
	prod := func(a,b int) int { return min(a,b) }

	// Case 1: Sort PP in descending order of pi, store pi+i,  query to the right, cand = queryval-(Pi+i)
	// Case 2: Sort PP in descending order of pi, store pi-i,  query to the left,  cand = queryval-(Pi-i)
	// Case 3: Sort PP in ascending  order of pi, store -pi+i, query to the right, cand = queryval-(-Pi+i)
	// Case 4: Sort PP in ascending  order of pi, store -pi-i, query to the left,  cand = queryval-(-Pi-i)

	sort.Slice(PP, func(i,j int) bool { return PP[i].pi > PP[j].pi })
	st1 := Newsegtree(N+5,prod,inf)
	st2 := Newsegtree(N+5,prod,inf)
	for _,pp := range PP { qval := st1.Prod(pp.i+1,N+2); D[pp.i] = min(D[pp.i],qval-(pp.pi+pp.i)); st1.Set(pp.i,pp.pi+pp.i) }
	for _,pp := range PP { qval := st2.Prod(0,pp.i-1);   D[pp.i] = min(D[pp.i],qval-(pp.pi-pp.i)); st2.Set(pp.i,pp.pi-pp.i) }
	sort.Slice(PP, func(i,j int) bool { return PP[i].pi < PP[j].pi })
	st3 := Newsegtree(N+5,prod,inf)
	st4 := Newsegtree(N+5,prod,inf)
	for _,pp := range PP { qval := st3.Prod(pp.i+1,N+2); D[pp.i] = min(D[pp.i],qval-(-pp.pi+pp.i)); st3.Set(pp.i,-pp.pi+pp.i) }
	for _,pp := range PP { qval := st4.Prod(0,pp.i-1);   D[pp.i] = min(D[pp.i],qval-(-pp.pi-pp.i)); st4.Set(pp.i,-pp.pi-pp.i) }
	ansstr := vecintstring(D[1:N+1])
	fmt.Println(ansstr)
}
