...
func solvePairs(v []int) []string {
	pairs := make([]string,0)
	i := 0; j := len(v)-1
	for i < j { pairs = append(pairs, fmt.Sprintf("%v %v",v[i],v[j])); i++; j-- }
	return pairs
}
...
func main() {
	...
	n2 := 2*M+1
	elems := make([]int,n2)
	for i:=0;i<n2;i++ { elems[i] = i+1 }
	pairs := make([]string,0)
	pairs = append(pairs,solvePairs(elems[:n2/2])...)
	pairs = append(pairs,solvePairs(elems[n2/2:])...)
	ansstr := strings.Join(pairs,"\n")
