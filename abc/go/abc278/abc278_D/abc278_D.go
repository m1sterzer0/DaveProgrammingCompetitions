...
type lstfunc struct {t,v int }

func lstop(a,b int) int { return min(a,b) }
func lstcomp(a,b lstfunc) lstfunc { if a.t == 0 { return a }; if b.t == 0 { return lstfunc{0,b.v+a.v} }; return lstfunc{1,b.v+a.v} }
func lstmap(f lstfunc, a int) int { if f.t == 0 { return f.v } else { return f.v + a } }

type lazysegtree struct {
	n, size, log int; op func(int, int) int; mapping func(lstfunc, int) int
	composition func(lstfunc, lstfunc) lstfunc; e int; id lstfunc; d []int; lz []lstfunc
}
func Newlazysegtree(n int, op func(int, int) int, mapping func(lstfunc, int) int, composition func(lstfunc, lstfunc) lstfunc, e int, id lstfunc) *lazysegtree {
	v := make([]int, n); for i := 0; i < n; i++ { v[i] = e }
	return NewlazysegtreeVec(v, op, mapping, composition, e, id)
}
func NewlazysegtreeVec(v []int, op func(int, int) int, mapping func(lstfunc, int) int, composition func(lstfunc, lstfunc) lstfunc, e int, id lstfunc) *lazysegtree {
...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); A := gis(N); Q := gi()
	lst := NewlazysegtreeVec(A,lstop,lstmap,lstcomp,1<<60,lstfunc{1,0})
	for i:=0;i<Q;i++ {
		t := gi()
		if t == 1 {
			x := gi(); lst.ApplyRange(0,N-1,lstfunc{0,x})
		} else if t == 2 {
			ii,x := gi(),gi(); lst.ApplyRange(ii-1,ii-1,lstfunc{1,x})
		} else {
			ii := gi(); ans := lst.Get(ii-1); fmt.Fprintln(wrtr,ans);
		}
	}
}
