...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M,E := gi(),gi(),gi(); U,V := fill2(E); Q := gi(); X := gis(Q)
	for i:=0;i<E;i++ { U[i]--; V[i]-- }; for i:=0;i<Q;i++ { X[i]-- }
	ansarr := ia(Q); startSb := make([]bool,E); for i:=0;i<E;i++ { startSb[i] = true }
	for _,x := range X { startSb[x] = false }
	uf := NewDsu(N+M+1)
	for i:=N;i<N+M;i++ { uf.Merge(i,N+M) }
	for i:=0;i<E;i++ { if startSb[i] { uf.Merge(U[i],V[i]) } }
	for i:=Q-1;i>=0;i-- {
		ansarr[i] = uf.Size(N+M) - M - 1
