...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); X := gis(N); C := gis(N)
	for i:=0;i<N;i++ { X[i]-- }
	numdeps := ia(N); for _,x := range X { numdeps[x]++ } 
	edges := make([]edge,0,N)
	for i:=0;i<N;i++ { edges = append(edges,edge{i,X[i],C[i]}) }
	sort.Slice(edges,func(i,j int) bool { return edges[i].c < edges[j].c } )
	q := make([]int,0,N) 
	for i:=0;i<N;i++ { if numdeps[i] == 0 { q = append(q,i) } }
	sb := make([]bool,N)
	ans,eidx := 0,0
	for {
		for len(q) > 0 {
			...
			sb[n] = true
			numdeps[X[n]]--
			if numdeps[X[n]] == 0 { q = append(q,X[n]) }
		}
		for eidx < N && (sb[edges[eidx].n1]) { eidx++ }
		if eidx == N { break }
		ans += edges[eidx].c // Cheapest way to break a loop 
		xn := edges[eidx].n2
		sb[xn] = true
		numdeps[X[xn]]--
		if numdeps[X[xn]] == 0 { q = append(q,X[xn]) }
		eidx++
	}
	fmt.Println(ans)
}
