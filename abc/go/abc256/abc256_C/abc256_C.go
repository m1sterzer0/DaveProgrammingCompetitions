...
func main() {
	...
	// PROGRAM STARTS HERE
	h1,h2,h3,w1,w2,w3 := gi(),gi(),gi(),gi(),gi(),gi()
	ans := 0
	for a:=1;a<=30;a++ {
		for b:=1;b<=30;b++ {
			for d:=1;d<=30;d++ {
				for e:=1;e<=30;e++ {
					c := h1 - a - b; if c <= 0 { continue }
					f := h2 - d - e; if f <= 0 { continue }
					g := w1 - a - d; if g <= 0 { continue }
					h := w2 - b - e; if h <= 0 { continue }
					i := w3 - c - f; if i <= 0 { continue }
					if g+h+i != h3 { continue }
					ans++
				}
			}
		}
