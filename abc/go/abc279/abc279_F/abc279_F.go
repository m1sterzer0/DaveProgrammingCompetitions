...
func main() {
	...
	// PROGRAM STARTS HERE
	N,Q := gi(),gi()
	// Numbers 1 through 1000000 represent boxes
	// Numbers 1000001 through 2000000 represent balls
	uf := NewDsu(2000001)
	nxtVirtBox := N+1
	nxtBall := 1000000+N+1
	leader2Box := ia(2000001); for i:=1;i<=N;i++ { leader2Box[i] = i; }
	box2CurrentVirtBox := ia(N+1); for i:=1;i<=N;i++ { box2CurrentVirtBox[i] = i }
	for i:=1;i<=N;i++ { l := uf.Merge(1000000+i,i); leader2Box[l]=i }
	for q:=0;q<Q;q++ {
		tt := gi()
		if tt == 1 {
			x,y := gi(),gi()
			l := uf.Merge(box2CurrentVirtBox[y],box2CurrentVirtBox[x]); leader2Box[l] = x
			box2CurrentVirtBox[y] = nxtVirtBox
			leader2Box[nxtVirtBox] = y
			nxtVirtBox++
		} else if tt == 2 {
			x := gi()
			l := uf.Merge(nxtBall,box2CurrentVirtBox[x]); leader2Box[l] = x
			nxtBall++
		} else {
			x := gi()
			l := uf.Leader(1000000+x)
			box := leader2Box[l]
			fmt.Fprintln(wrtr,box)
		}
	}
}
