...
func main() {
	...
	for t:=0;t<T;t++ {
		...
		zg,og := make([][]int,N),make([][]int,N)
		for i:=0;i<M;i++ { 
			u,v := U[i],V[i]
			if C[u] == 0 { zg[v] = append(zg[v],u) } else { og[v] = append(og[v],u) }
			if C[v] == 0 { zg[u] = append(zg[u],v) } else { og[u] = append(og[u],v) }
		}
		inf := 1<<60; darr := twodi(N,N,inf); darr[0][N-1] = 0; q := Newqueue(); q.Push(pair{0,N-1})
		for !q.IsEmpty() {
			p := q.Pop(); a,b := p.a,p.b
			for _,i := range zg[a] {
				for _,j := range og[b] {
		...
		ans := darr[N-1][0]; if ans == inf { ans = -1 }; fmt.Println(ans)
	}
}
