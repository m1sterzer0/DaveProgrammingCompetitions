...
func main() {
	...
	// Main trick is to create 2 virtual nodes -- one for an airport hub and one for a harbor hub
	// We then run Kruskal 4 times  and pick the best
	// N is airport, N+1 is harbor
	edges := make([]edge,0,M+2*N)
	for i:=0;i<M;i++ { edges = append(edges,edge{A[i],B[i],Z[i]}) }
	for i:=0;i<N;i++ { edges = append(edges,edge{i,N,X[i]})}
	for i:=0;i<N;i++ { edges = append(edges,edge{i,N+1,Y[i]})}
	sort.Slice(edges,func(i,j int) bool { return edges[i].c < edges[j].c } )
	ans := 1<<60
	barr := []bool{true,false}
	for _,aflag := range barr {
		for _,hflag := range barr {
			lans := 0
			uf := NewDsu(N+2)
			for _,e := range edges {
				if e.b == N && !aflag || e.b == N+1 && !hflag { continue }
				if uf.Same(e.a,e.b) { continue }
				lans += e.c; uf.Merge(e.a,e.b)
			}
			// Now check to see if we got everything
			targsize := N
			if aflag { targsize++ }
			if hflag { targsize++ }
			if uf.Size(0) != targsize { continue }
			ans = min(ans,lans)
		}
	}
	fmt.Println(ans)
