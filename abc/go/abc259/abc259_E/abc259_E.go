...
func main() {
	...
	for i:=0;i<N;i++ {
		...
		for j:=0;j<m;j++ {
			p,e := gi(),gi()
			parr = append(parr,pf{p,e,i})
		}
	}
	sb := make([]bool,N)
	sort.Slice(parr,func(i,j int) bool { return parr[i].p < parr[j].p || parr[i].p == parr[j].p && parr[i].e < parr[j].e } )
	idx := 0; nn := len(parr)
	for idx < nn {
		p := parr[idx].p
		for idx < nn && parr[idx].p == p { idx++ }
		if idx == 1 || parr[idx-1].p != parr[idx-2].p || parr[idx-1].e != parr[idx-2].e { sb[parr[idx-1].idx] = true }
	}
	ans := 0
	for _,b := range sb { if !b { ans++; break } }
	for _,b := range sb { if b { ans++ } }
	fmt.Println(ans)
}
