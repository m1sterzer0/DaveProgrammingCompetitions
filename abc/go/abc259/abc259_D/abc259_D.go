...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); Sx,Sy,Tx,Ty := gi(),gi(),gi(),gi(); X,Y,R := fill3(N)
	// Just use union find
	uf := NewDsu(N+2)
	// Do s & t
	for i:=0;i<N;i++ {
		if (Sx-X[i])*(Sx-X[i])+(Sy-Y[i])*(Sy-Y[i]) == R[i]*R[i] { uf.Merge(i,N)   }
		if (Tx-X[i])*(Tx-X[i])+(Ty-Y[i])*(Ty-Y[i]) == R[i]*R[i] { uf.Merge(i,N+1) }
	} 
	// Do each pair of circles
	for i:=0;i<N;i++ {
		for j:=i+1;j<N;j++ {
			// Check if a circle is inside the other one
			if (X[i]-X[j])*(X[i]-X[j])+(Y[i]-Y[j])*(Y[i]-Y[j]) < (R[j]-R[i])*(R[j]-R[i]) { continue }
			// Check if the centers are too far apart
			if (X[i]-X[j])*(X[i]-X[j])+(Y[i]-Y[j])*(Y[i]-Y[j]) > (R[j]+R[i])*(R[j]+R[i]) { continue }
			uf.Merge(i,j)
		}
	}
	ans := "No"
	if uf.Same(N,N+1) { ans = "Yes" }
	fmt.Println(ans)
}
