...
type seg struct { c byte; cnt int}
func main() {
	...
	// PROGRAM STARTS HERE
	S := gs(); T := gs()
	encodeStr := func(s string) []seg {
		idx := 0
		ansarr := []seg{}
		for idx < len(s) {
			c := s[idx]; cnt := 0
			for idx < len(s) && s[idx] == c { cnt++; idx++ }
			ansarr = append(ansarr,seg{c,cnt})
		}
		return ansarr
	}
	aa := encodeStr(S)
	...
	if len(aa) != len(bb) {
		...
		for i:=0;i<len(aa);i++ {
			if aa[i].c != bb[i].c    { ans = "No"; break }
			if aa[i].cnt > bb[i].cnt { ans = "No"; break }
			if aa[i].cnt == 1 && bb[i].cnt > 1 { ans = "No"; break }
		}
	}
	fmt.Println(ans)
}
