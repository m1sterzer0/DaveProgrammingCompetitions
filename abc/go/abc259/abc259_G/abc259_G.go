...
func main() {
	...
	rowsum := ia(H); colsum := ia(W)
	for i:=0;i<H;i++ { for j:=0;j<W;j++ { rowsum[i] += A[i][j]; colsum[j] += A[i][j] } }
	ans := 0
	for _,r := range rowsum { if r > 0 { ans += r } }
	for _,c := range colsum { if c > 0 { ans += c } }
	mf := NewMfgraph(H+W+2)
	for i:=0;i<H;i++ { if rowsum[i] > 0 { mf.Addedge(H+W,i,rowsum[i]) } }
	for j:=0;j<W;j++ { if colsum[j] > 0 { mf.Addedge(H+j,H+W+1,colsum[j]) } }
	for i:=0;i<H;i++ {
		for j:=0;j<W;j++ {
			v := A[i][j]; if v < 0 { v = inf }
			mf.Addedge(i,H+j,v)
		}
	}
	f := mf.Flow(H+W,H+W+1)
