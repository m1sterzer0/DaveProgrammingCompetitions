...
func main() {
	...
	cnt := 0; d := make(map[string]int)
	NS,NT := ia(N),ia(N)
	for i,s := range S { v,ok := d[s]; if !ok { v = cnt; cnt++; d[s] = v }; NS[i] = v }
	for i,t := range T { v,ok := d[t]; if !ok { v = cnt; cnt++; d[t] = v }; NT[i] = v }
	sidx := iai(2*N,-1); for i,s := range NS { sidx[s] = i }
	dep := make([]PI,0)
	for i,t := range NT { if sidx[t] >= 0 { dep = append(dep,PI{sidx[t],i}) } }
	counter,_ := Kosaraju(N,dep)
	ans := "Yes"; if counter != N { ans = "No" }
	fmt.Println(ans)
}
