...
func main() {
	...
	// PROGRAM STARTS HERE
	// Generally want to buy low and sell high -- trick here is delayed choice OR partial booking of sales
	// We will go with the partial booking here
	N := gi(); P := gis(N)
	mh := Newminheap(func(a,b int) bool { return a < b } ); ans := 0;
	for _,a := range P {
		if mh.Len() > 0 && a > mh.Head() { ans += a-mh.Head(); mh.Pop(); mh.Push(a); mh.Push(a) } else { mh.Push(a) }
	}
	fmt.Println(ans)
}
