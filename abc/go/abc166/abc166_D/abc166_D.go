...
func main() {
	...
	//1001**5 - 1000**5 is 5_010_010_005_001, so A and abs(B) are both less than 1001.  Also, WLOG, can assume A > 0 and abs(A) >= abs(B).
	// Dump approach -- build a "pow5" array, and blindly iterate on A,B 
	X := gti()
	pow5 := make([]int,1001)
	for i:=0;i<=1000;i++ { pow5[i] = i*i*i*i*i }
	ansa := 0; ansb := 0
	for a:=1; a<=1000; a++ {
		for b:=0; b<=a; b++ {
			if X == pow5[a]-pow5[b] { ansa,ansb = a,b  }
			if X == pow5[a]+pow5[b] { ansa,ansb = a,-b }
		}
	}
