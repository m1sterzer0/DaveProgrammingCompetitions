...
func main() {
	...
	mha := Newminheap(func(a,b int) bool { return a > b })
	mhb := Newminheap(func(a,b int) bool { return a > b })
	for _,a := range A { mha.Push(a) }
	for _,b := range B { mhb.Push(b) }
	ans := 0
	for !mha.IsEmpty() {
		if mha.Head() == mhb.Head() { mha.Pop(); mhb.Pop(); continue }
		if mhb.Head() > mha.Head() {
			b := mhb.Pop()
