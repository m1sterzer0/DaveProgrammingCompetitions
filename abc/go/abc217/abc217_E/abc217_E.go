...
func main() {
	...
	// PROGRAM STARTS HERE
	Q := gi(); A := ia(Q); B := ia(Q); for i:=0;i<Q;i++ { A[i] = gi(); if A[i] == 1 { B[i] = gi()} }
	dq := Newdeque(); mh := Newminheap(func(a,b int) bool { return a < b })
	for i:=0;i<Q;i++ {
		a,b := A[i],B[i]
		if a == 1 { 
			dq.PushBack(b)
		} else if a == 2 {
			if !mh.IsEmpty() { fmt.Fprintln(wrtr,mh.Pop()) } else {fmt.Fprintln(wrtr,dq.PopFront()) }
		} else {
			for !dq.IsEmpty() { mh.Push(dq.PopBack()) }
		}
	}
}
