...
func main() {
	...
	// PROGRAM STARTS HERE
	N,K,Q := gi(),gi(),gi(); A := gis(K); L := gis(Q)
	bd := make([]bool,N+1)
	for _,a := range A { bd[a] = true }
	for _,l := range L {
		ii := 0; cnt := 0
		for cnt < l { ii++; if bd[ii] { cnt++ } }
		if ii < N && bd[ii+1] == false { bd[ii] = false; bd[ii+1] = true }
	}
	ansarr := make([]int,0,K); for i:=1;i<=N;i++ { if bd[i] { ansarr = append(ansarr,i) } }
