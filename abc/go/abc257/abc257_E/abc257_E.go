...
func main() {
	...
	// PROGRAM STARTS HERE
	// clearly need to maximize the number of digits
	N := gi(); C := gis(9); minc := minarr(C); numdig := N / minc
	cidx := 8; for C[cidx] != minc { cidx-- }
	ansarr := make([]byte,numdig); for i:=0;i<numdig;i++ { ansarr[i] = '1'+byte(cidx) }
	budgetLeft := N - numdig * minc
	for i:=0;i<numdig;i++ {
		imp := false
		for c:=8; c>cidx; c-- {
			if (C[c]-minc) <= budgetLeft { imp = true; ansarr[i] = '1'+byte(c); budgetLeft-= (C[c]-minc); break }
		}
		if !imp { break } 
	}
	fmt.Println(string(ansarr))
}
