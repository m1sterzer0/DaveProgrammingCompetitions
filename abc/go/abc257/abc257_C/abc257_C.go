...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); S := gs(); W := gis(N)
	type pers struct { w,t int }
	P := make([]pers,0,N)
	for i:=0;i<N;i++ { t := 0; if S[i] == '1' { t = 1 }; P = append(P,pers{W[i],t}) }
	sort.Slice(P, func(i,j int) bool { return P[i].w < P[j].w } )
	totadults := 0; for _,p := range P { if p.t == 1 { totadults++ } }
	best := totadults; cur := totadults
	idx := 0
	for idx < N {
		w := P[idx].w
		for idx < N && P[idx].w == w { if P[idx].t == 0 { cur++ } else { cur-- }; idx++ }
		best = max(best,cur)
	}
	fmt.Println(best)
}
