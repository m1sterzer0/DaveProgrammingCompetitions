...
func main() {
	...
	for len(q) > 0 {
		n := q[0]; q = q[1:]
		if n == 0 { continue }
		for _,n2 := range gr[n] { if dfrom1[n2] == inf { dfrom1[n2] = dfrom1[n]+1; q = append(q,n2) } }
	}
	dfromN[N] = 0; q = append(q,N)
	for len(q) > 0 {
		n := q[0]; q = q[1:]
		if n == 0 { continue }
		for _,n2 := range gr[n] { if dfromN[n2] == inf { dfromN[n2] = dfromN[n]+1; q = append(q,n2) } }
	}
	ansarr := make([]int,0,N)
	for i:=1;i<=N;i++ {
		ans := min(min(dfrom1[N],dfrom1[0]+dfromN[i]),min(dfrom1[i]+dfromN[0],dfrom1[0]+dfromN[0]))
		if ans == inf { ans = -1 }
