...
def getrbtreeMap(classname,keytype,valuetype) :
	template = '''
	package oc
type RBTREEMAPnode struct { left, right, up int32; red bool; key KEYTYPE; val VALTYPE }
type RBTREEMAP struct {
	lessthan func(a, b KEYTYPE) bool; tree []RBTREEMAPnode; root int32; recycler []int32; sz int; minidx int32
	maxidx int32
}
type RBTREEMAPIterator interface { Next() (ok bool); Prev() (ok bool); Key() KEYTYPE; Value() VALTYPE }
type RBTREEMAPiter struct { cur int32; key KEYTYPE; value VALTYPE; rbtree *RBTREEMAP }
...
func NewRBTREEMAP(lessthan func(a, b KEYTYPE) bool) *RBTREEMAP {
	q := &RBTREEMAP{lessthan, make([]RBTREEMAPnode, 2), int32(0), make([]int32, 0), 0, 0, 0}; q.tree[0].left = 0
	q.tree[0].right = 0; q.tree[0].up = 0; q.tree[0].red = false; q.recycler = append(q.recycler, 1); return q
}
func (q *RBTREEMAP) Add(k KEYTYPE, v VALTYPE) {
	if q.sz == 0 {
		z := q.getNewNodenum(); tree := q.tree; q.minidx = z; q.maxidx = z; q.sz += 1; q.root = z
		tree[z].key, tree[z].val, tree[z].up, tree[z].left, tree[z].right, tree[z].red = k, v, 0, 0, 0, false; return
	}; y, x, tree, lt, lastleft := int32(0), q.root, q.tree, q.lessthan, false; for x != 0 {
		y = x; if lt(k, tree[x].key) { x = tree[x].left; lastleft = true } else if lt(tree[x].key, k) {
			x = tree[x].right; lastleft = false
		} else { tree[x].val = v; return }
	}; z := q.getNewNodenum(); q.sz += 1; tree = q.tree
	tree[z].key, tree[z].val, tree[z].up, tree[z].left, tree[z].right, tree[z].red = k, v, y, 0, 0, true; if y == 0 {
		q.root = z
	} else if lastleft { tree[y].left = z } else { tree[y].right = z }
	...
	for p = tree[z].up; tree[p].red; p = tree[z].up { 
		g = tree[p].up ; if g == 0 { break } ; if p == tree[g].left {
			u = tree[g].right; if tree[u].red {
				tree[p].red = false; tree[u].red = false; tree[g].red = true; z = g; continue
			}; if z == tree[p].right { z = p; q.rotleft(z); p = tree[z].up }; q.rotright(g); tree[g].red = true
			tree[p].red = false
		} else { 
			u = tree[g].left; if tree[u].red {
				tree[p].red = false; tree[u].red = false; tree[g].red = true; z = g; continue
			}; if z == tree[p].left { z = p; q.rotright(z); p = tree[z].up }; q.rotleft(g); tree[g].red = true
			tree[p].red = false
		}
...
func (q *RBTREEMAP) Delete(k KEYTYPE) bool {
	...
	q.recycler = append(q.recycler, z); if q.sz > 0 && !q.lessthan(q.tree[q.minidx].key, k) {
		q.minidx = q.nextidx(q.minidx)
	}; if q.sz > 0 && !q.lessthan(k, q.tree[q.maxidx].key) { q.maxidx = q.previdx(q.maxidx) }; if q.sz == 0 {
		q.root = 0; return true
	}; tree := q.tree; var x int32; y, y_orig_red := z, tree[z].red; if tree[z].left == 0 {
		x = tree[z].right; q.rbTransplant(z, x)
	} else if tree[z].right == 0 { x = tree[z].left; q.rbTransplant(z, x) } else {
		y = q.findminidx(tree[z].right); y_orig_red = tree[y].red; x = tree[y].right; if tree[y].up == z {
			tree[x].up = y 
		} else { q.rbTransplant(y, x); tree[y].right = tree[z].right; tree[tree[y].right].up = y }; q.rbTransplant(z, y)
		tree[y].left = tree[z].left; tree[tree[y].left].up = y; tree[y].red = tree[z].red
	}; if !y_orig_red {
		for q.root != x && !tree[x].red {
			p := tree[x].up; if tree[p].left == x {
				s := tree[p].right ; if tree[s].red {
					tree[s].red = false; tree[p].red = true; q.rotleft(p); s = tree[p].right
				}; c := tree[s].left; d := tree[s].right; if !tree[c].red && !tree[d].red {
					tree[s].red = true; x = p
				} else {
				...
				s := tree[p].left ; if tree[s].red {
					tree[s].red = false; tree[p].red = true; q.rotright(p); s = tree[p].left
				}; c := tree[s].right; d := tree[s].left; if !tree[c].red && !tree[d].red {
					tree[s].red = true; x = p
				} else {
...
func (q *RBTREEMAP) Lookup(k KEYTYPE) (VALTYPE, bool) {
	var def VALTYPE; z, cmp := q.findInsertionPoint(k); if cmp == 0 { return q.tree[z].val, true }; return def, false
}
func (q *RBTREEMAP) Len() int { return q.sz }
func (q *RBTREEMAP) MinKey() (k KEYTYPE) {
	if q.sz == 0 { panic("Called MinKey on an empty RBTREEMAP") }; return q.tree[q.minidx].key
}
func (q *RBTREEMAP) MaxKey() (k KEYTYPE) {
	if q.sz == 0 { panic("Called MaxKey on an empty RBTREEMAP") }; return q.tree[q.maxidx].key
}
func (q *RBTREEMAP) LowerBound(k KEYTYPE) (KEYTYPE, bool) {
	var def KEYTYPE; if q.sz == 0 { return def, false }; idx, pos := q.findInsertionPoint(k); if pos == 1 {
		idx = q.nextidx(idx)
	}; if idx <= 0 { return def, false }; return q.tree[idx].key, true
}
...
func (q *RBTREEMAP) LowerBoundIter(k KEYTYPE) (RBTREEMAPIterator, bool) {
	...
	if idx <= 0 { return nil, false }; return &RBTREEMAPiter{idx, q.tree[idx].key, q.tree[idx].val, q}, true
}
...
func (q *RBTREEMAP) rbTransplant(u, v int32) {
	tree := q.tree; if tree[u].up == 0 { q.root = v } else {
		p := tree[u].up; if u == tree[p].left { tree[p].left = v } else { tree[p].right = v }
	}; tree[v].up = tree[u].up
}
func (q *RBTREEMAP) findInsertionPoint(k KEYTYPE) (int32, int8) {
	n, lt, tree := q.root, q.lessthan, q.tree; for {
		nkey := tree[n].key; if lt(nkey, k) {
			r := tree[n].right; if r == 0 { return n, 1 }; n = r
		} else if lt(k, nkey) { l := tree[n].left; if l == 0 { return n, -1 }; n = l } else { return n, 0 }
	}
}
...
func (q *RBTREEMAP) nextidx(cur int32) int32 {
	last := int32(-2); tree := q.tree; rr := tree[cur].right; if rr > 0 { return q.findminidx(rr) }; for {
		last, cur = cur, tree[cur].up; if cur == 0 || tree[cur].left == last { break }
	}; return cur
}
func (q *RBTREEMAP) previdx(cur int32) int32 {
	last := int32(0); tree := q.tree; ll := tree[cur].left; if ll > 0 { return q.findmaxidx(ll) }; for {
		last, cur = cur, tree[cur].up; if cur == 0 || tree[cur].right == last { break }
	}; return cur
}
func (q *RBTREEMAP) rotleft(x int32) {
	tree := q.tree; y := tree[x].right; p := tree[x].up; tree[x].right = tree[y].left; if tree[y].left != 0 {
		tree[tree[y].left].up = x
	}; tree[y].up = p; if p == 0 { q.root = y } else if x == tree[p].left { tree[p].left = y } else {
		tree[p].right = y
	}; tree[y].left = x; tree[x].up = y
}
func (q *RBTREEMAP) rotright(x int32) {
	tree := q.tree; y := tree[x].left; p := tree[x].up; tree[x].left = tree[y].right; if tree[y].right != 0 {
		tree[tree[y].right].up = x
	}; tree[y].up = p; if p == 0 { q.root = y } else if x == tree[p].right { tree[p].right = y } else {
		tree[p].left = y
	}; tree[y].right = x; tree[x].up = y
}
func (q *RBTREEMAP) getNewNodenum() int32 {
	l := len(q.recycler); newnode := q.recycler[l-1]; q.recycler = q.recycler[:l-1]; if l == 1 {
		q.tree = append(q.tree, RBTREEMAPnode{}); q.recycler = append(q.recycler, int32(len(q.tree)-1))
	}; return newnode
}
package oc
type RBTREEMAPnode struct { left, right, up int32; red bool; key KEYTYPE; val VALTYPE }
...
'''
	template = template.replace("RBTREEMAP",classname)
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) > 1 :
        ...
        elif sys.argv[1] == "rbtreemap" and len(sys.argv) == 6 :        s = getrbtreeMap(sys.argv[2],sys.argv[3],sys.argv[4]); appendStringToFile(s,sys.argv[5])
        else : good = False
