...
func isum(a,b int) int { return a + b }
type ss struct {i,v,m int}
...
func main() {
	...
	AA := make([]ss,N); for i,a := range A { AA[i] = ss{i,a+i,-1} }
	BB := make([]ss,N); for i,b := range B { BB[i] = ss{i,b+i,-1} }
	sort.Slice(AA,func(i,j int) bool { return AA[i].v < AA[j].v || AA[i].v == AA[j].v  && AA[i].i < AA[j].i} )
	sort.Slice(BB,func(i,j int) bool { return BB[i].v < BB[j].v || BB[i].v == BB[j].v  && BB[i].i < BB[j].i} )
	ans := 0
	for i:=0;i<N;i++ {
		if AA[i].v != BB[i].v { ans = -1; break }
		BB[i].m = AA[i].i
	}
	if ans == 0 {
		sort.Slice(BB,func(i,j int) bool { return BB[i].i < BB[j].i } )
		pos := make([]int,N); for i:=0;i<N;i++ { pos[i] = i }
		lst := NewlazysegtreeVec(pos,max,isum,isum,0,0)
		for i:=0;i<N;i++ { m := BB[i].m; p := lst.Get(m); ans += p-i; lst.ApplyRange(0,m,1) }
	}
	fmt.Println(ans)
}
