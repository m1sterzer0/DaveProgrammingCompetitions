...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); P := gis(N); for i:=0;i<N;i++ { P[i]-- }; ops := make([]op,0,5*N)
	curs := 0
	for curs < N {
		v := P[curs]
		if v == curs { curs++; continue }
		if curs < N>>1 {
			if v < N>>1 {
				ops = append(ops,op{curs,N-1},op{N-1,v},op{curs,N-1})
			} else if v == N-1 {
				ops = append(ops,op{curs,N-1})
			} else {
				ops = append(ops,op{curs,N-1},op{N-1,0},op{0,v},op{N-1,0},op{curs,N-1})
			}
		} else {
			if v >= N>>1 {
				ops = append(ops,op{curs,0},op{0,v},op{curs,0})
			} else if v == 0 {
				ops = append(ops,op{curs,0})
			} else {
				ops = append(ops,op{curs,0},op{N-1,0},op{N-1,v},op{N-1,0},op{curs,0})
			}
		}
		P[curs],P[v] = P[v],P[curs]
	}
