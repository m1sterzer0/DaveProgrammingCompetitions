...
func main() {
	...
	// Have to try random
	N := gi(); V,M := fill264(N)
	sorig := sumarr64(V); if sorig < 0 { for i:=0;i<N;i++ { V[i] *= -1 } }
	gr := make([][]obj,63)
	for i:=0;i<N;i++ { p := 63-bits.LeadingZeros64(uint64(M[i])); gr[p] = append(gr[p],obj{V[i],M[i]}) }
	ans := int64(0)
	for i:=0;i<63;i++ {
		s := int64(0)
		for _,p := range gr[i] {
			if bits.OnesCount64(uint64(p.m & ans)) & 1 == 0 { s += p.v } else { s -= p.v }
		}
		if s > 0 { ans |= int64(1) << uint(i) }
	}
	fmt.Println(ans)
}
