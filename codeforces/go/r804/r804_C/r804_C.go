...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		l,r := aloc[0],aloc[0]; avail := 0; mex := 1
		sb := make([]bool,N); sb[0] = true
		ans := int64(1)
		for l > 0 || r < N-1 {
			idx := aloc[mex]
			if idx > r { for i:=r+1; i<=idx; i++ { sb[A[i]] = true; avail++ }; r = idx }
			if idx < l { for i:=l-1; i>=idx; i-- { sb[A[i]] = true; avail++ }; l = idx }
			newmex := mex; for newmex < N && sb[newmex] { newmex++ }
			avail-- // for the current mex
			if newmex > mex+1 {
				numneeded := newmex-mex-1
				for i:=0;i<numneeded;i++ { ans *= int64(avail); ans %= MOD; avail-- }
			}
			mex = newmex
		}
