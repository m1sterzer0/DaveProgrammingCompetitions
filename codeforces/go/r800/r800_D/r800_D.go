...
func main() {
	...
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N := gi(); P := gis(N-1); for i:=0;i<N-1;i++ { P[i]-- }; L,R := fill2(N)
		gr := make([][]int,N)
		for i:=1;i<N;i++ { p := P[i-1]; gr[p] = append(gr[p],i) }
		var dfs func(n int) (int,int64)
		dfs = func(n int) (int,int64) {
			if len(gr[n]) == 0 {
				return 1,int64(R[n])
			} else {
				cnt,sum := 0,int64(0)
				for _,c := range gr[n] { ccnt,ccmaxsum := dfs(c); cnt += ccnt; sum += ccmaxsum }
				if sum > int64(R[n]) { return cnt,int64(R[n]) }
				if sum < int64(L[n]) { return cnt+1,int64(R[n]) }
				return cnt,sum
			}
		}
		nops,_ := dfs(0)
		fmt.Fprintln(wrtr,nops)
	}
}
