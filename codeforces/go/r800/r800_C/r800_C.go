...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		incarr,decarr := make([]int64,N),make([]int64,N)
		ans := "Yes"
		incarr[0] = int64(A[0])
		for i:=1;i<N;i++ {
			decarr[i] = incarr[i-1]
			incarr[i] = int64(A[i]) + decarr[i]
		}
		if incarr[N-1] != 0 { ans = "No" }
		for _,a := range incarr { if a < 0 { ans = "No" } }
		for _,a := range decarr { if a < 0 { ans = "No" } }
		foundZero := false
		for _,a := range incarr { if a == 0 { foundZero = true } else if foundZero { ans = "No" } }
