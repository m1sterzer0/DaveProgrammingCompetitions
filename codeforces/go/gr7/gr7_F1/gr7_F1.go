...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); adjm := make([]string,N); for i:=0;i<N;i++ { adjm[i] = gs() }
	// dp[subset mask][last element][ansmask] = ways
	dp := make([][][]int64,1<<uint(N))
	for i:=0;i<1<<uint(N);i++ { dp[i] = make([][]int64,N) }
	for i:=1;i<1<<uint(N);i++ { 
		for j:=0;j<N;j++ { 
			if (i >> uint(j)) & 1 == 0 { continue }
			nb := bits.OnesCount(uint(i))
			if nb == 1 { dp[i][j] = append(dp[i][j],1); continue } // Special case for single bits
			dp[i][j] = make([]int64,1<<(uint(nb)-1))
			lastm := i ^ (1<<uint(j))
			for k:=0;k<N;k++ {
				if (lastm >> uint(k)) & 1 == 0 { continue }
				lastv := 0; if adjm[j][k] == '1' { lastv = 1 }
				for s,v := range dp[lastm][k] {	dp[i][j][2*s+lastv] += v }
			} 
		}
	}
	fullm := (1<<uint(N))-1
	ansarr := make([]int64,1<<uint(N-1))
	for i:=0;i<1<<uint(N-1);i++ {
		for k:=0;k<N;k++ {
			ansarr[i] += dp[fullm][k][i]
		}
	}
