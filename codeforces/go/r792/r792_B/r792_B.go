...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		a,b,c := gi(),gi(),gi()
		// z = c, y = b, x = a+nb s.t. x > c, so a+nb > c --> nb > (c-a)
		n := (c-a)/b + 1; x := a + n*b; y := b; z := c; 
		//if x % y != a { fmt.Fprintf(wrtr,"ERROR x mod y = %v mod %v = %v != %v == a\n",x,y,x%y,a) }
		//if y % z != b { fmt.Fprintf(wrtr,"ERROR y mod z = %v mod %v = %v != %v == b\n",y,z,y%z,b) }
		//if z % x != c { fmt.Fprintf(wrtr,"ERROR z mod x = %v mod %v = %v != %v == c\n",z,x,z%x,c) }
