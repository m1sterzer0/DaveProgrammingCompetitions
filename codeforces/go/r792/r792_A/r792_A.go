...
func main() {
	...
	// PROGRAM STARTS HERE
	// two digit number -- forced to take rightmost digit
	// three digit number -- can get left or right but not middle
	// four digit number -- can get any of the digits
	T := gi()
	for tt:=1;tt<=T;tt++ { 
		s := gs(); ansarr := make([]byte,1)
		if len(s) == 2 {
			ansarr[0] = s[1]
		} else {
			ansarr[0] = s[0]
			for _,c := range s { if byte(c) < ansarr[0] { ansarr[0] = byte(c) } }
		}
