...
func main() {
	...
	// PROGRAM STARTS HERE
	// Any singleton greater than the mex should be used to help the mex (guaranteed to reduce cost by at least 1)
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N,K := gi(),gi(); A := gis(N)
		hist := make(map[int]int)
		for _,a := range A { hist[a]++ }
		aa := Newrbtreemultiset(func(a,b int) bool { return a < b })
		bb := Newrbtreemultiset(func(a,b int) bool { return a < b })
