...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		for i,c := range S {
			if c == 'L' { continue }
			if i == 0 || S[i-1] == 'L' { raw += 1 } else { raw += 2 }
		}
		// If all of S are losses, the best we can do is 2*K-1
		nw,nl := 0,0; for _,c := range S { if c == 'W' { nw++ } else { nl++ } }
		ans := 0
		if K == 0 {
			ans = raw
		} else if nw == 0 {
			...
			ans = raw
			gaps := make([]int,0); cgap := 0
			for i,c := range S {
				if c == 'W' && cgap > 0 {
					gaps = append(gaps,cgap); cgap = 0
				} else if c == 'L' && cgap > 0 {
					cgap++
				} else if c == 'L' && i > 0 && S[i-1] == 'W' {
					cgap=1
				}
			}
			sort.Slice(gaps,func(i,j int) bool { return gaps[i] < gaps[j] })
			for _,g := range gaps { if g <= K {  K -= g; ans += 2*g+1 } }
			ans += 2*K
		}
