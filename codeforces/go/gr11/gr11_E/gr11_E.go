...
func main() {
	...
	for gcd64(X,(n*X) ^ X) != 1 { ansarr = append(ansarr,fmt.Sprintf("%v + %v",n*X,n*X)); n *= 2 }
	ansarr = append(ansarr,fmt.Sprintf("%v ^ %v",n*X,X))
	x := X; y := (n*X) ^ X
	_,a,b := gcdExtended64(x,y)

	if a < 0 { a,x,b,y = b,y,a,x }
	b *= -1
	if b % 2 == 1 { b += x; a += y } // Want ax - by == 1 with even by
	doit := func(a,x int64) {
		res := int64(0); v := x; p := int64(1)
		for {
			if a & p != 0 {	if res == 0 { res = v } else { ansarr = append(ansarr,fmt.Sprintf("%v + %v",res,v)); res += v } }
			if res == a*x { break } 
			ansarr = append(ansarr,fmt.Sprintf("%v + %v",v,v)); v += v
			p = p << uint(1)
		}
	}
	doit(a,x);
	doit(b,y);
	ansarr = append(ansarr,fmt.Sprintf("%v ^ %v",a*x,b*y))
