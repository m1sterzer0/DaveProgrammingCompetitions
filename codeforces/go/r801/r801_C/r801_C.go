...
func main() {
	...
	T := gi();
	for tt:=1;tt<=T;tt++ {
		N,M := gi(),gi(); gr := make([][]int,N); for i:=0;i<N;i++ { gr[i] = gis(M); }
		// Just need to check if zero is in between the max and min and has the right parity
		dpmax := twodi(N,M,-999999999)
		dpmin := twodi(N,M,999999999)
		dpmax[0][0] = gr[0][0]
		dpmin[0][0] = gr[0][0]
		for i:=0;i<N;i++ {
			for j:=0;j<M;j++ {
				if i > 0 { dpmax[i][j] = max(dpmax[i][j],dpmax[i-1][j]+gr[i][j]); dpmin[i][j] = min(dpmin[i][j],dpmin[i-1][j]+gr[i][j]) }
				if j > 0 { dpmax[i][j] = max(dpmax[i][j],dpmax[i][j-1]+gr[i][j]); dpmin[i][j] = min(dpmin[i][j],dpmin[i][j-1]+gr[i][j]) }
			}
		}
		ans := "NO"
		if (N+M) % 2 == 1 && dpmax[N-1][M-1] >= 0 && dpmin[N-1][M-1] <= 0 { ans = "YES" }
