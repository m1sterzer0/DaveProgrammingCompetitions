...
func main() {
	...
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N := gi(); X,Y := fill2(N-1); for i:=0;i<N-1;i++ { X[i]--; Y[i]-- }
		gr := make([][]int,N)
		...
		dfs = func(n,p int) {
			sumc[n] = 0; numz[n] = 0; ansarr[n] = 0
			for _,c := range gr[n] {
				if c == p { continue }
				dfs(c,n);
				sumc[n] += ansarr[c]; if ansarr[c] == 0 { numz[n]++ }	
			}
			ansarr[n] = sumc[n]
			if numz[n] > 1 { ansarr[n] += numz[n]-1 }
		}
		reroot := func(oldp,oldc int) {
			// Take away oldc from oldp
			sumc[oldp] -= ansarr[oldc]
			if ansarr[oldc] == 0 { numz[oldp]-- }
			ansarr[oldp] = sumc[oldp]
			if numz[oldp] > 1 { ansarr[oldp] += numz[oldp]-1 }

			// add oldp to oldc
			sumc[oldc] += ansarr[oldp]
			if ansarr[oldp] == 0 { numz[oldc]++ }
			ansarr[oldc] = sumc[oldc]
			if numz[oldc] > 1 { ansarr[oldc] += numz[oldc]-1 }
		}
		dfs2 = func(n,p int) {
			best = min(best,ansarr[n])
			for _,c := range gr[n] {
				if c == p { continue }
				reroot(n,c); dfs2(c,n);	reroot(c,n)
			}
		}
		dfs(0,-1)
		dfs2(0,-1)
