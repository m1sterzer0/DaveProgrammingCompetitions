...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		// If there are an odd number of piles, then the first person to play always wins, because he takes all the stones from the
		// first pile, and when it comes back around to the first pile, the other player has no stones to take.
		//
		// If there are an even number of piles, then any pile is only ever "drawn from" by one player.  An optimal strategy here
		// is just to take one stone when you have to draw in order to prolong the game.  The loser is the player that has to draw from the 
		// "first minimum" pile
		ans := "Mike"
		if N % 2 == 0 {
			m,mi := 1000000001,-1
			for i,c := range A { if c < m { m,mi = c,i } }
			if mi % 2 == 0 { ans = "Joe" }
		}
