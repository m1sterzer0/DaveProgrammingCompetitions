...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi()
		gr := make([]string,N); for i:=0;i<N;i++ { gr[i] = gs() }
		xcnt := []int{0,0,0}; ycnt := []int{0,0,0}
		for i:=0;i<N;i++ {
			for j:=0;j<N;j++ {
				if gr[i][j] == 'X' { xcnt[(i+j)%3]++ }
				if gr[i][j] == 'O' { ycnt[(i+j)%3]++ }
			}
		}
		// choices are (0,1), (0,2), (1,0), (1,2), (2,0), (2,1)
		best := N*N; xidx := -1; oidx := -1
		type pair struct {i,j int}
		for _,p := range []pair{{0,1},{0,2},{1,0},{1,2},{2,0},{2,1}} {
			if xcnt[p.i] + ycnt[p.j] < best { best = xcnt[p.i] + ycnt[p.j]; xidx = p.i; oidx = p.j }
		}
		ansarr := make([][]byte,N); for i:=0;i<N;i++ { ansarr[i] = make([]byte,N) }
		for i:=0;i<N;i++ {
			for j:=0;j<N;j++ {
				if gr[i][j] == '.' {
					ansarr[i][j] = '.'
				} else if gr[i][j] == 'X' {
					if (i+j) % 3 == xidx { ansarr[i][j] = 'O' } else { ansarr[i][j] = 'X' }
				} else {
					if (i+j) % 3 == oidx { ansarr[i][j] = 'X' } else { ansarr[i][j] = 'O' }
				}
			}
		}
		for i:=0;i<N;i++ {
