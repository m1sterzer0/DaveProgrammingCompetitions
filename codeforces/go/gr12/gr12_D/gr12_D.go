...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		cnt := 0; l := 0; r := N-1
		for cnt < N {
			if hist[cnt+1] == 0 { break }
			if hist[cnt+1] > 1 { cnt++; break }
			if hist[cnt+1] == 1 {
				if A[l] == cnt+1 { l++; cnt++; continue }
				if A[r] == cnt+1 { r--; cnt++; continue }
				cnt++; break 
			}
		} 
		ansarr := make([]byte,N)
		// First case is special
		ansarr[0] = '1'; for i:=1;i<=N;i++ { if hist[i] != 1 { ansarr[0] = '0'; break } }
		for i:=1;i<N;i++ { if cnt >= N-i { ansarr[i] = '1' } else { ansarr[i] = '0' } }
