...
func main() {
	...
	mina := minarr(A)
	st := twodi(20,N,inf)
	for i,a := range(A) { st[0][i] = a }
	for r:=1;r<19;r++ {
		prev,cur,g := st[r-1],st[r],1<<uint(r-1)
		for i:=0;i<N;i++ { ridx := (i+g)%N; cur[i] = min(prev[i],prev[ridx]) }
	}
	rmq := func(i,l int) int { 
		idx := 63-bits.LeadingZeros64(uint64(l))
		ll := 1 << uint(idx)
		s2 := (i + l-ll ) % N
		ans := min(st[idx][i],st[idx][s2])
		//fmt.Printf("     RMQ(%v,%v)=%v\n",i,l,ans)
		return ans
	}
	ansarr := make([]int,N)
	for i,a := range A {
		//fmt.Printf("DBG: EVALUATING i,a := %v,%v\n",i,a)
		if 2*mina >= a {
			ansarr[i] = inf
		} else { 
			l,u := 1,N
	...
	for c:=0;c<2;c++ {
		for i:=N-1;i>=0;i-- {
			ansarr[i] = min(ansarr[i],1+ansarr[(i+1)%N])
		}
	}
	for i:=0;i<N;i++ { if ansarr[i] == inf { ansarr[i] = -1 } }
