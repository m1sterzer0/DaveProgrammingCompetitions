...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); X,Y,Z := fill3(N)
	p := make([]pt,N); for i:=0;i<N;i++ { p[i] = pt{X[i],Y[i],Z[i],i+1} }
	sort.Slice(p, func(i,j int) bool { return p[i].x < p[j].x || p[i].x == p[j].x && (p[i].y < p[j].y || p[i].y == p[j].y && p[i].z < p[j].z) } )
	sb := make([]bool,N)
	// Pass 1 : Look for any pairs we can make with p[i].x == p[j].x && p[i].y == p[j].y
	idx1,idx2 := 0,0
	for idx1 < N { 
		for idx1 < N && sb[idx1] { idx1++ }; idx2 = idx1+1; for idx2 < N && sb[idx2] { idx2++ }; if idx2 >= N { break }
		if p[idx1].x == p[idx2].x && p[idx1].y == p[idx2].y { fmt.Fprintf(wrtr,"%v %v\n",p[idx1].idx,p[idx2].idx); sb[idx1]=true; sb[idx2] = true }
		idx1 = idx2
	}
	// Pass 2 : Look for any pairs we can make with p[i].x == p[j].x
	idx1,idx2 = 0,0
	for idx1 < N { 
		for idx1 < N && sb[idx1] { idx1++ }; idx2 = idx1+1; for idx2 < N && sb[idx2] { idx2++ }; if idx2 >= N { break }
		if p[idx1].x == p[idx2].x { fmt.Fprintf(wrtr,"%v %v\n",p[idx1].idx,p[idx2].idx); sb[idx1]=true; sb[idx2] = true }
		idx1 = idx2
	}
	// Pass 3 : Pair up the rest in order of x coordinate
	idx1,idx2 = 0,0
	for idx1 < N { 
		for idx1 < N && sb[idx1] { idx1++ }; idx2 = idx1+1; for idx2 < N && sb[idx2] { idx2++ }; if idx2 >= N { break }
		if true { fmt.Fprintf(wrtr,"%v %v\n",p[idx1].idx,p[idx2].idx); sb[idx1]=true; sb[idx2] = true }
		idx1 = idx2
	}
}
