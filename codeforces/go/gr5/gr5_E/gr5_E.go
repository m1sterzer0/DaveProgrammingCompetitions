...
func main() {
	...
	// PROGRAM STARTS HERE
	// For each depth, there are only two valid subtrees (conclude 4&5 are only depth 3 solutions and induct)
	// Even one will always have even subtrees on both left and right
	// Odd one will always have left subtree odd and right subtree even
	N := gi()
	good := []int{1,2,4,5}
	idx := 3
	for good[idx] < N && good[idx-1] < N { good = append(good,1+good[idx-1]+good[idx],1+good[idx-1]+good[idx-1]); idx += 2 }
	ans := 0
	if N == 1 || N == 2 || N == good[idx-1] || N == good[idx] { ans = 1 }
	fmt.Println(ans) 
}
