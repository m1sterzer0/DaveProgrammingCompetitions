...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		if N % 2 == 1 { fmt.Fprintln(wrtr,"NO"); continue }
		sort.Slice(A,func(i,j int) bool { return A[i] < A[j] } )
		lidx,ridx := 0,N/2
		b := make([]int,0,N); for i:=0;i<N/2;i++ { b = append(b,A[lidx],A[ridx]); lidx++; ridx++ }
		// Now for the check
		good := true
		for i:=0;i<N;i++ { 
			l := tern(i==0,N-1,i-1)
			r := tern(i==N-1,0,i+1)
			if !(b[i] > b[l] && b[i] > b[r] || b[i] < b[l] && b[i] < b[r]) { good = false; break }
		}
		if !good { fmt.Fprintln(wrtr,"NO"); } else { fmt.Fprintf(wrtr,"YES\n%v\n",vecintstring(b)) }
	}
}
