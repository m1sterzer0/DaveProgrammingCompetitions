...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); A := gis(N); Q := gi(); S,T,U := fill3(Q); for i:=0;i<Q;i++ { S[i]--; U[i]-- }
	milestones := make(map[pair]int)
	free := ia(N)
	ans := int64(0); for _,a := range(A) { ans += int64(a) }
	for i:=0;i<Q;i++ {
		s,t,u := S[i],T[i],U[i]
		v,ok := milestones[pair{s,t}]
		if ok {
			if free[v] <= A[v] { ans++ }
			free[v]--
			delete(milestones,pair{s,t})
		}
		if u >= 0 {
			if free[u] < A[u] { ans-- }
			free[u]++
			milestones[pair{s,t}] = u
		}
