...
type debt struct { u,v int; d int64 }
func main() {
	...
	// PROGRAM STARTS HERE
	N,M := gi(),gi(); U,V,D := fill3(M)
	sb := ia64(N+1)
	for i:=0;i<M;i++ { u,v,d := U[i],V[i],D[i]; sb[u] += int64(d); sb[v] -= int64(d) }
	// Now we just match up the people that are net positive debt vs net negative debt
	ii,jj := 1,1; debts := make([]debt,0)
	for ii <= N && jj <= N {
		for ii <= N && sb[ii] <= 0 { ii++ }
		for jj <= N && sb[jj] >= 0 { jj++ }
		if ii > N && jj > N { break }
		xx := min64(sb[ii],-sb[jj])
		debts = append(debts,debt{ii,jj,xx})
		sb[ii] -= xx; sb[jj] += xx
	}
	fmt.Fprintln(wrtr,len(debts))
	for _,d := range debts { fmt.Fprintf(wrtr,"%v %v %v\n",d.u,d.v,d.d)}
}
