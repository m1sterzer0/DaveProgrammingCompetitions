...
func main() {
	...
	N,M,Q := gi(),gi(),gi()
	bd := make([][]byte,N)
	for i:=I(0);i<N;i++ { bd[i] = gbs() }
	X,Y := fill2(Q); for i:=I(0);i<Q;i++ { X[i]--; Y[i]-- }
	nicons := I(0)
	for i:=I(0);i<N;i++ { for j:=I(0);j<M;j++ { if bd[i][j] == '*' { nicons++ }}}
	bc,br := nicons/N,nicons % N // First non-icon
	goodicons := I(0)
	for i:=I(0);i<N;i++ { for j:=I(0);j<M;j++ { if (j < bc || j == bc && i < br) && bd[i][j] == '*' { goodicons++ } } }
	add := func(ii,jj int64) {
		// Add element -- check good -- expand good -- check added element
		bd[ii][jj] = '*'
		nicons++
		if jj < bc || jj == bc && ii < br { goodicons++ }
		if bc < M && bd[br][bc] == '*' { goodicons++ }
		br++; if br==N { bc++; br = 0}
	}
	sub := func(ii,jj int64) {
		bd[ii][jj] = '.'
		nicons--
		if jj < bc || jj == bc && ii < br { goodicons-- }
		br--; if br < 0 { bc--; br = N-1}
		if bd[br][bc] == '*' { goodicons-- }
	}
	for i:=I(0);i<Q;i++ {
		x,y := X[i],Y[i]
		if bd[x][y] == '*' { sub(x,y) } else { add(x,y) }
		fmt.Fprintln(wrtr,nicons-goodicons)
	}
}
