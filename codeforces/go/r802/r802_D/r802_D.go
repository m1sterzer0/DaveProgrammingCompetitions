...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); V := gis64(N); Q := gi(); T := gis64(Q)
	// Calculate the minimum time to fill up the first n locks with n pipes
	arr := iai64(N+1,0); s := int64(0)
	for i:=1;i<=N;i++ { s += V[i-1]; t := (s+int64(i)-1)/int64(i); arr[i] = max64(arr[i-1],t) }
	// Calculate the minimum time to fill up all n locks with n pipes assuming they were all at the beginning
	for i:=1;i<=N;i++ { arr[i] = max64(arr[i],(s+int64(i)-1)/int64(i) ) }
	// Iterate through the queries and use binary search to find the answer
	for _,t := range T {
		ans := -1
		if arr[N] <= t { 
			l,u := 0,N;
			for u-l > 1 { m := (u+l)>>1; if arr[m] <= t { u = m } else { l = m } }
			ans = u
		}
