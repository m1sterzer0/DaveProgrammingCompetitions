...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M := gi(),gi(); bd := make([][]int,N); for i:=0;i<N;i++ { bd[i] = gis(M); }
	ll := make([]loc,M*N+1)
	for i:=0;i<N;i++ { for j:=0;j<M;j++ { ll[bd[i][j]] = loc{i,j} } }

	check := func(v int) bool {
		if v == 1 { return true }
		ii,jj := ll[v].i,ll[v].j
		if (ii > 0 && bd[ii-1][jj] < v) || (ii+1 < N && bd[ii+1][jj] < v) { return true }
		if (jj > 0 && bd[ii][jj-1] < v) || (jj+1 < M && bd[ii][jj+1] < v) { return true }
		return false
	}

	badarr := make([]int,0)
	for i:=1;i<=N*M;i++ { if !check(i) { badarr = append(badarr,i) } } 
	if len(badarr) == 0 { fmt.Println(0); return }

	swaps := 0
	adjbad := []int{badarr[0]}
	ii,jj := ll[badarr[0]].i, ll[badarr[0]].j;
	if ii > 0 { adjbad = append(adjbad,bd[ii-1][jj]) }
	if jj > 0 { adjbad = append(adjbad,bd[ii][jj-1]) }
	if ii+1 < N { adjbad = append(adjbad,bd[ii+1][jj]) }
	if jj+1 < M { adjbad = append(adjbad,bd[ii][jj+1]) }

	visited := make(map[pair]bool)
	for _,v1 := range adjbad {
		for v2:=1;v2<=N*M;v2++ {
			t1,t2 := min(v1,v2),max(v1,v2); if visited[pair{t1,t2}] { continue }; visited[pair{t1,t2}] = true
			// check that all of the bad numbers are within a manhattan distance of 1 of at least one of the swaps
			ii1,jj1,ii2,jj2 := ll[v1].i,ll[v1].j,ll[v2].i,ll[v2].j
			good := true
			for _,v := range badarr {
				ii3,jj3 := ll[v].i,ll[v].j
				if abs(ii3-ii1)+abs(jj3-jj1) > 1 && abs(ii3-ii2)+abs(jj3-jj2) > 1 { good = false; break }
			}
			if !good { continue }
			// Do the swap 
			bd[ii1][jj1],bd[ii2][jj2] = bd[ii2][jj2],bd[ii1][jj1]; ll[v1],ll[v2] = ll[v2],ll[v1]
			//Inventory the 10 points we need to check
			if !check(v1) || !check(v2) { good = false }
			if ii1 > 0   && !check(bd[ii1-1][jj1]) { good = false }
			if ii2 > 0   && !check(bd[ii2-1][jj2]) { good = false }
			if ii1+1 < N && !check(bd[ii1+1][jj1]) { good = false }
			if ii2+1 < N && !check(bd[ii2+1][jj2]) { good = false }
			if jj1 > 0   && !check(bd[ii1][jj1-1]) { good = false }
			if jj2 > 0   && !check(bd[ii2][jj2-1]) { good = false }
			if jj1+1 < M && !check(bd[ii1][jj1+1]) { good = false }
			if jj2+1 < M && !check(bd[ii2][jj2+1]) { good = false }
			bd[ii1][jj1],bd[ii2][jj2] = bd[ii2][jj2],bd[ii1][jj1]; ll[v1],ll[v2] = ll[v2],ll[v1]
			if good { swaps++ }
		}
	}

	if swaps == 0 { fmt.Fprintln(wrtr,2) } else { fmt.Fprintf(wrtr,"%v %v\n",1,swaps) }
}
