...
func main() {
	...
	// PROGRAM STARTS HERE -- following the solution
	N := gi()
	st := make([][]int,2); en := make([][]int,2)
	st[0] = gis(N); st[1] = gis(N); en[0] = gis(N); en[1] = gis(N)
	if sumarr(st[0])+sumarr(st[1]) != sumarr(en[0]) + sumarr(en[1]) { fmt.Println(-1); return }
	d1,d2 := 0,0; ans := int64(0)
	for i:=0;i<N;i++ {
		d1 += st[0][i]-en[0][i]
		d2 += st[1][i]-en[1][i]
		if d1 > 0 && d2 < 0 { ans += 1; d1--; d2++ }
		if d1 < 0 && d2 > 0 { ans += 1; d1++; d2-- }
		ans += int64(abs(d1)) + int64(abs(d2))
	}
	fmt.Println(ans)
}
