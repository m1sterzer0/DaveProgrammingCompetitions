...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi()
	gr := make([][]int64,N); for i:=0;i<N;i++ { gr[i] = make([]int64,N) }
	for i:=1;i<N;i+=2 {
		for j:=0;j<N;j++ { gr[i][j] = int64(1) << uint(i-1+j) }
	}
	for i:=0;i<N;i++ { fmt.Fprintln(wrtr,vecint64string(gr[i])) }
	wrtr.Flush()
	Q := gi()
	for i:=0;i<Q;i++ {
		k := gi64()
		cx,cy := 0,0; fmt.Fprintf(wrtr,"%v %v\n",cx+1,cy+1)
		for i:=0;i<2*N-2;i++ {
			if cx == N-1 { 
				cy++
			} else if cy == N-1 {
				cx++
			} else if gr[cx][cy+1] > 0 && k & gr[cx][cy+1] != 0 {
				cy++
			} else if gr[cx+1][cy] > 0 && k & gr[cx+1][cy] != 0 {
				cx++
			} else if gr[cx][cy+1] == 0 {
				cy++
			} else {
				cx++
			}
