...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M,K := gi(),gi(),gi(); S,T := gbs(),gbs(); A,B := fill2(N); for i:=0;i<N;i++ { A[i]--; B[i]-- }

	// Permutation we use to track the inverse permutation
	p := ia(K); for i:=0;i<K;i++ {p[i] = i }
	getMask := func(s []byte) int { bm := 0; for i:=0;i<K;i++ { if s[i] == '1' { bm |= 1 << uint(i) } }; return bm }
	ws := make([]byte,K)
	wt := make([]byte,K)
	inf := 1000000000
	dps := iai(1<<uint(K),inf); dpt := iai(1<<uint(K),-inf)
	dps[getMask(S)] = 0; dpt[getMask(T)] = 0
	for i:=0;i<N;i++ {
		a,b := A[i],B[i]
		p[a],p[b] = p[b],p[a]
		for j:=0;j<K;j++ {
		...
		ms,mt := getMask(ws),getMask(wt)
		dps[ms] = min(dps[ms],i+1)
		dpt[mt] = i+1
	}
	ans,bestl,bestr := 0,1,N
	o1 := bits.OnesCount(uint(getMask(S))); o2 := bits.OnesCount(uint(getMask(T)))
	for bm := (1<<uint(K))-1;bm>=0;bm-- {
		bmones := bits.OnesCount(uint(bm))
		cand := K - (o1-bmones) - (o2-bmones)
		if cand <= ans { continue }
		if dpt[bm] - dps[bm] >= M { ans = cand ; bestl = dps[bm]+1; bestr = dpt[bm] }
		for j:=0;j<K;j++ {
			if bm & (1<<uint(j)) == 0 { continue }
			newbm := bm ^ (1<<uint(j))
			dps[newbm] = min(dps[newbm],dps[bm])
			dpt[newbm] = max(dpt[newbm],dpt[bm])
		}
	}
	fmt.Println(ans)
	fmt.Printf("%v %v\n",bestl,bestr)
