...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		// If all equal, then we can't do anything, otherwise, we use (a) largest number to swallow everybody else
		alleq := true; for i:=0;i<N-1;i++ { if A[i] != A[i+1] { alleq = false } }
		v := 1; if alleq { v = N }; fmt.Fprintln(wrtr,v)
	}
}
