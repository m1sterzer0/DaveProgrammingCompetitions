...
func main() {
	...
	N := gi(); H := gis64(N)
	sh := sumarr64(H)
	l,u := int64(-1),int64(1000000000001)
	for u-l > 1 { m := (u+l)>>1; mv := int64(N) * (m + m - 1 + int64(N)) / 2; if mv >= sh { u = m } else { l = m } }
	basesum := (u + u + int64(N) - 2) * (int64(N-1)) / 2
	ansarr := []int64{sh-basesum}; for i:=0;i<N-1;i++ { ansarr = append(ansarr,u+int64(i)) }
	sort.Slice(ansarr,func(i,j int) bool { return ansarr[i] < ansarr[j] })
