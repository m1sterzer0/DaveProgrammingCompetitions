...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); S := gbs()
		fl,fr := false,false; for _,c := range S { if c == 'L' { fl = true } else { fr = true } }
		ans := 0; if !fl { ans++; S[0] = 'L' } else if !fr { ans++; S[0] = 'R' }
		// First, check for all left or all right
		st := 1; for S[st] == S[0] { st++ }
		offset := 0
		for offset < N {
			v := S[(st+offset)%N]
			l := 0
			for S[(st+offset)%N] == v { l++; offset++ }
			ans += l/3
		}
