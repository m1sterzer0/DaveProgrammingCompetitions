...
func main() {
	...
	// PROGRAM STARTS HERE
	N,H := gi(),gi(); A := gis(N)
	working := make([]int,0,N+1)
	try := func(n int) bool {
		working = working[:0]
		for i:=0;i<n;i++ { working = append(working,A[i]) }
		sort.Slice(working,func(i,j int) bool { return working[i] > working[j] } )
		cum := 0
		for i:=0;i<n;i+=2 {
			if i+1 < n { cum += max(working[i],working[i+1]) } else { cum += working[i] }
			if cum > H { return false }
		}
		return true
	} 
	l,u := 0,N+1; for u-l > 1 { m := (l+u)>>1; if try(m) { l = m } else { u = m } }
	fmt.Println(l)
}
