...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); S := make([]int64,N); for i:=0;i<N;i++ { S[i] = gi64() }
	Q := gi(); L,R := make([]int64,Q),make([]int64,Q); for i:=0;i<Q;i++ { L[i],R[i] = gi64(),gi64() }
	ansarr := make([]int64,Q)
	if N == 1 {
		for i:=0;i<Q;i++ { ansarr[i] = R[i]-L[i]+1 }
	} else {
		sort.Slice(S,func(i,j int) bool { return S[i] < S[j] } )
		gaps := make([]int64,N-1); for i:=0;i<N-1;i++ { gaps[i] = S[i+1]-S[i] }
		sort.Slice(gaps,func(i,j int) bool { return gaps[i] < gaps[j] } )
		cumgaps := make([]int64,N-1); cumgaps[0] = gaps[0]; for i:=1;i<N-1;i++ { cumgaps[i] = cumgaps[i-1] + gaps[i] }
		for i:=0;i<Q;i++ {
			g := R[i]-L[i]
			l,u := -1,N-1; for u-l > 1 { m := (u+l)>>1; if gaps[m] <= g { l = m } else { u = m } }
			if l == -1 { ansarr[i] = (R[i]-L[i]+1) * int64(N) } else { nmerged := 2+l; ansarr[i] = int64(R[i]-L[i]+1) * int64(N-nmerged) + cumgaps[l] + R[i]-L[i] + 1 } 
		}
	}
	fmt.Println(vecint64string(ansarr))
}
