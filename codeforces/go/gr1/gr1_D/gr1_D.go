...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M := gi(),gi(); A := gis(N)
	sb := ia(M+10); for _,a := range A { sb[a]++ }
	dp := twodi(3,3,0); ndp := twodi(3,3,0)
	for i:=3;i<=M+3;i++ {
		for j:=0;j<=2;j++ { for k:=0;k<=2;k++ { ndp[j][k] = 0 } }
		for j:=0;j<=2;j++ { 
			for k:=0;k<=2;k++ { 
				c1,c2,c3 := sb[i-2]-j-k,sb[i-1]-k,sb[i]
				if c1 < 0 || c2 < 0 || c3 < 0 { continue }
				ndp[k][0] = max(ndp[k][0],dp[j][k]+c1/3)
				if c1 >= 1 && c2 >= 1 && c3 >= 1 { ndp[k][1] = max(ndp[k][1],dp[j][k]+1+(c1-1)/3 ) }
				if c1 >= 2 && c2 >= 2 && c3 >= 2 { ndp[k][2] = max(ndp[k][2],dp[j][k]+2+(c1-2)/3 ) }
			} 
		}
		dp,ndp = ndp,dp
	}
