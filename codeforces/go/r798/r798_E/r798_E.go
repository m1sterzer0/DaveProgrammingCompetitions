...
func main() {
	...
	// PROGRAM STARTS HERE
	// Obs 1: Must add 1 to all of the zero elements
	// Obs 2: After this, the answer is 0, 1, or 2, so we check for 0 and 1, and if none of those are correct, we report 2 (plus the zero conversions of course)
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		adder := 0; for i:=0;i<N;i++ { if A[i] == 0 { adder++; A[i] = 1 } }
		dosol := func(xx int) { fmt.Fprintf(wrtr,"%v\n%v\n",xx,vecintstring(A)) }
		check := func() bool {
			uf := NewDsu(N)
			for i:=uint(0);i<=30;i++ {
				bm := 1<<i; first := -1
				for j,a := range A { 
					if a & bm == 0 { continue }
					if first == -1 { first = j } else { uf.Merge(first,j) }
				}
			}
			return uf.Size(0) == N
		}
		if check() { dosol(0+adder); continue }
		foundone := false
		for i:=0;i<N;i++ { 
			A[i] -= 1; if check() { dosol(1+adder); foundone = true; break }
			A[i] += 2; if check() { dosol(1+adder); foundone = true; break }
			A[i] -= 1
		}
		if foundone { continue }
		maxtz := 0; for _,a := range A { maxtz = max(maxtz,bits.TrailingZeros(uint(a))) }
		b := ia(0); for i,a := range A  { if bits.TrailingZeros(uint(a)) == maxtz { b = append(b,i) } }
		// We should have at least two entries here in b
		A[b[0]]--; A[b[1]]++; dosol(2+adder)
	}
