...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		maxd1,mind1,maxd2,mind2 := -inf,inf,-inf,inf
		maxdist,ii,jj := inf,-1,-1
		for i:=0;i<N;i++ {
			for j:=0;j<M;j++ {
				if bd[i][j] == 'B' { maxd1 = max(maxd1,i+j); mind1 = min(mind1,i+j); maxd2 = max(maxd2,i-j); mind2 = min(mind2,i-j) }
			}
		}
		for i:=0;i<N;i++ {
			for j:=0;j<M;j++ {
				d := 0; d = max(d,abs(i+j-maxd1)); d = max(d,abs(i+j-mind1)); d = max(d,abs(i-j-maxd2)); d = max(d,abs(i-j-mind2))
				if d < maxdist { maxdist,ii,jj = d,i,j}
			}
		}
