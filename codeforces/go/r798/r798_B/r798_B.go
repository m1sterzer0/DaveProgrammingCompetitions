...
func main() {
	...
	// PROGRAM STARTS HERE
	T := gi()
	ss := Newrbtreeset(func (a,b int) bool { return a < b } )
	for tt:=1;tt<=T;tt++ {
		N := gi(); P := gis(N)
		if N == 1 { fmt.Fprintln(wrtr,-1); continue }
		ss.Clear(); for i:=1;i<=N;i++ { ss.Add(i) }
		ansarr := ia(N)
		for i:=0;i<N-1;i++ {
			p := P[i]
			m := ss.MinKey()
			if m == p { m,_ = ss.FindGt(m) }
			ansarr[i] = m; ss.Delete(m)
		}
		m,_ := ss.FindGt(0); ansarr[N-1] = m
		if P[N-1] == ansarr[N-1] { ansarr[N-2],ansarr[N-1] = ansarr[N-1],ansarr[N-2] }
