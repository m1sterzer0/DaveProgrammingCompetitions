...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		...
		dfs = func(n,p int) {
			...
			child = child[:0]
			sz[n] = 1; saved[n] = 0
			for _,c := range gr[n] { if c == p { continue }; sz[n] += sz[c]; child = append(child,c) }
			if len(child) == 1 { 
				saved[n] = sz[child[0]]-1
			} else if len(child) == 2 {
				saved[n] = max(sz[child[0]]-1+saved[child[1]],sz[child[1]]-1+saved[child[0]])
			}
			return
		}
		dfs(0,-1)
		fmt.Fprintln(wrtr,saved[0])
	}
}
