...
func main() {
	...
	gi(); // N
	M := gi()
	// First M queries, we get the value of each of the edges
	sarr := make([]byte,M); for i:=0;i<M;i++ { sarr[i] = '0' }
	type edge struct {idx,v int }
	edges := make([]edge,0,M)
	for i:=0;i<M;i++ {
		sarr[i] = '1'
		s := string(sarr)
	...
	for _,e := range edges {
		...
		fmt.Fprintf(wrtr,"? %v\n",s); wrtr.Flush()
		val := gi()
		if ans + e.v != val { sarr[e.idx] = '0' } else { ans += e.v }
	}
	fmt.Fprintf(wrtr,"! %v\n",ans); wrtr.Flush()
}
