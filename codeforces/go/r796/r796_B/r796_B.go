...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		ocnt,ecnt,mintz := 0,0,999
		for _,a := range A { mintz = min(mintz,bits.TrailingZeros(uint(a))); if a & 1 == 0 { ecnt++ } else { ocnt++ } }
		ans := 0; if ocnt > 0 { ans = ecnt } else { ans = ecnt - 1 + mintz }
