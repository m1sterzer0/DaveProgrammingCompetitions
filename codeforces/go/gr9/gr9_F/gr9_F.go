...
func main() {
	...
	// PROGRAM STARTS HERE
	A,B,C := gi64(),gi64(),gi64()
	fmt.Fprintln(wrtr,"First"); wrtr.Flush()
	for {
		// Check for an arithmetic sequence where you can't use the top
		maxv := max64(A,max64(B,C))
		minv := min64(A,min64(B,C))
		midv := A+B+C-minv-maxv
		v := 3*maxv-A-B-C
		if midv-minv == maxv-midv { v = midv-minv }
		fmt.Fprintln(wrtr,v); wrtr.Flush()
		idx := gi()
		if idx <= 0 { break }
		if idx == 1 { A += v } else if idx == 2 { B += v } else { C += v }
	}
}
