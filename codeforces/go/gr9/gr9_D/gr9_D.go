...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		hist := ia(N+1); for _,a := range A { hist[a]++ }
		good := make([]bool,N); goodcnt := 0
		for i:=0;i<N;i++ { if A[i] == i { good[i] = true; goodcnt++ } }
		ansarr := make([]int,0,2*N)
		for goodcnt < N {
			mex := 0; for hist[mex] > 0 { mex++ }
			if mex != N {
				ansarr = append(ansarr,mex+1)
				hist[A[mex]]--; hist[mex]++; A[mex] = mex; good[mex] = true; goodcnt++
			} else {
				// Find a bad entry to move to a value of N
				idx := 0; for good[idx] { idx++ }
				ansarr = append(ansarr,idx+1)
				hist[A[idx]]--; hist[N]++; A[idx] = N
			}
		}
