...
func main() {
	...
	// PROGRAM STARTS HERE
	N := gi(); A := gis(N)
	type en struct { a,idx int }
	AA := make([]en,N); for i:=0;i<N;i++ { AA[i] = en{A[i],i} }
	sort.Slice(AA,func(i,j int) bool { return AA[i].a < AA[j].a} )
	...
	for aidx < N {
		v := AA[aidx].a
		for aidx < N && AA[aidx].a == v { aidx++ }
		w1 = w1[:0]; w2 = w2[:0]
		for i,a := range A { if a == v { w2 = append(w2,i) } else if a < v { for _,w := range w2 { w1 = append(w1,pair{w,i} ) } } }
		sort.Slice(w1,func(i,j int) bool { return w1[i].x < w1[j].x || w1[i].x == w1[j].x && w1[i].y > w1[j].y } )
		for _,w := range w1 { ansarr = append(ansarr,pair{w.x+1,w.y+1}) }
	}
