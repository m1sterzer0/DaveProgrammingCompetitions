...
func main() {
	...
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N := gi(); S := gi(); A := gis(N); for i:=0;i<N;i++ { A[i]-- }; B := gis(N); for i:=0;i<N;i++ { B[i]-- }
		Aloc := ia(N); for i,a := range A { Aloc[a] = i }
		// Figure out which elements of B are left
		sb := make([]bool,N); for _,b := range B { if b >= 0 { sb[b] = true } }
		// Check B to make sure that things are possible
		good := true
		for i,b := range B { if b >= 0 && A[i] > b+S { good = false } }
		ans := int64(1)
		if !good {
			ans = 0
		} else {
			amoves := ia(0); for i := 0;i<N;i++ { if B[i] < 0 { amoves = append(amoves,A[i]) } }
			bmoves := ia(0); for i := 0;i<N;i++ { if !sb[i]   { bmoves = append(bmoves,i) } }
			sort.Slice(amoves,func(i,j int) bool { return amoves[i] < amoves[j] } )
			avail := 0; aidx := 0;
			for _,b := range bmoves {
				for aidx < len(amoves) && amoves[aidx] <= b+S { avail++; aidx++ }
				ans *= int64(avail); ans %= MOD; avail--
			}
		}
