...
func main() {
	...
	// PROGRAM STARTS HERE
	// If the array has 3 positive numbers or 3 negative numbers, then it cannot be closed
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N := gi(); A := gis(N)
		z := 0; nz := ia(0); for _,a := range A { if a == 0 { z++ } else { nz = append(nz,a) } }
		good := true
		if len(nz) > 4 {
			good = false 
		} else {
			sb := make(map[int64]bool)
			if z > 0 { sb[0] = true }
			for _,x := range nz { sb[int64(x)] = true }
			if z > 0 {
				for i:=0;i<len(nz);i++ {
					for j:=i+1;j<len(nz);j++ {
						s := int64(nz[i])+int64(nz[j])
						if !sb[s] { good = false }
					}
				}
			}
			for i:=0;i<len(nz);i++ {
				for j:=i+1;j<len(nz);j++ {
					for k:=j+1;k<len(nz);k++ {
						s := int64(nz[i])+int64(nz[j])+int64(nz[k])
						if !sb[s] { good = false }
					}
				}
			}
