...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M := gi(),gi(); C := gis(M); for i:=0;i<M;i++ { C[i]-- }

	// We need to do segment compression
	bd := make([]int,0,M)
	for i:=0;i<M;i++ { if i == 0 || C[i] != C[i-1] { bd = append(bd,C[i]) } }
	ans := int64(0); NN := len(bd)
	if NN <= 2*N+2 { // Too many segments
		lidx,ridx := ia(NN),ia(NN)
		for i:=0;i<NN;i++    { ridx[bd[i]] = i }
		for i:=NN-1;i>=0;i-- { lidx[bd[i]] = i }
		dp := twodi64(NN,NN,0)
		for sz:=1;sz<=NN;sz++ {
			for l:=0;l+sz<=NN;l++ {
				r:=l+sz-1
				countWays := func(ll,rr int) int64 {
					if rr < ll { return 1 }
					ans := int64(0)
					if ll <= rr { ans += 2 * dp[ll][rr] }
					for m := ll; m < rr; m++ { ans += dp[ll][m] * dp[m+1][rr] % MOD }
					return ans % MOD
				}
				mval := bd[l]; for i:=l;i<=r;i++ { mval = min(mval,bd[i]) }
				cl,cr := lidx[mval],ridx[mval]
				if l > cl || r < cr { 
					dp[l][r] = 0
				} else if cl == l && cr == r {
					ans := int64(1)
					lastidx := l
					for i:=l+1;i<=r;i++ { if bd[i] == mval { ans = ans * dp[lastidx+1][i-1] % MOD; lastidx = i } }
					dp[l][r] = ans
				} else {
					left,right := countWays(l,cl-1),countWays(cr+1,r)
					dp[l][r] = left * right % MOD * dp[cl][cr] % MOD
				}
				//fmt.Printf("DBG: dp[%v][%v] = %v\n",l,r,dp[l][r] )
			}
		}
		ans = dp[0][NN-1]
	}
