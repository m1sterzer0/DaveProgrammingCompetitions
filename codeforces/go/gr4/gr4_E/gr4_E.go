...
func main() {
	...
	// PROGRAM STARTS HERE
	S := gs()
	a1 := make([]byte,0,250000)
	i,j := 0,len(S)-1
	numquads := len(S)/4
	for k:=0;k<numquads;k++ {
		c := byte('a'); if S[i]==S[j] || S[i] == S[j-1] { c = byte(S[i]) } else { c = byte(S[i+1]) }
		a1 = append(a1,c)
		i+=2;j-=2
	}
	a2 := make([]byte,len(a1)); copy(a2,a1); ii,jj := 0,len(a2)-1; for ii<jj { a2[ii],a2[jj] = a2[jj],a2[ii]; ii++; jj-- }
	if i <= j { a1 = append(a1,S[i]) }
	ans := string(a1) + string(a2)
	fmt.Println(ans)
}
