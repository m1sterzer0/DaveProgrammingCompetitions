...
func main() {
	...
	T := gi()
	for tt:=1;tt<=T;tt++ {
		N,M := gi(),gi(); X,Y := fill2(M); for i:=0;i<M;i++ { X[i]--; Y[i]-- }
		gr := make([][]int,N); for i:=0;i<M;i++ { x,y := X[i],Y[i]; gr[x] = append(gr[x],y) }
		for i:=0;i<N;i++ { if len(gr[i]) == 2 && gr[i][0] > gr[i][1] { gr[i][0],gr[i][1] = gr[i][1],gr[i][0] } }
		sink   := make([]bool,N)
		bad    := make([]bool,N)
		for i:=0;i<N;i++ {
			if bad[i] { continue } 
			if sink[i] {
				for _,j := range gr[i] { bad[j] = true }
			} else {
				for _,j := range gr[i] { sink[j] = true }
			}
		}
		closed := make([]int,0,N)
		for i:=0;i<N;i++ { if bad[i] { closed = append(closed,i+1) } }
		fmt.Fprintln(wrtr,len(closed))
		fmt.Fprintln(wrtr,vecintstring(closed))
	}
}
