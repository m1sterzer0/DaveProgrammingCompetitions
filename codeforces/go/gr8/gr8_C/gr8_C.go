...
func main() {
	...
	// PROGRAM STARTS HERE
	K := gi()
	bd := make([][]bool,5); for i:=0;i<5;i++ { bd[i] = make([]bool,2*K+3)}
	for i:=0;i<2*K+3;i++ { bd[2][i] = true }
	for i:=0;i<K;i++ { bd[1][2+2*i] = true; bd[3][2+2*i] = true }
	// Connect the left to top left stub
	bd[1][0] = true; bd[0][0] = true; bd[0][1] = true; bd[0][2] = true
	// Connect the pairs on the top
	for i:=1;i+1<K;i+=2 { bd[0][2+2*i] = true; bd[0][3+2*i] = true; bd[0][4+2*i] = true }
	// Connect the pairs on the bottom
	for i:=0;i+1<K;i+=2 { bd[4][2+2*i] = true; bd[4][3+2*i] = true; bd[4][4+2*i] = true }
	// Connect the right to the spare endpoint
	if K % 2 == 0 {
		bd[1][2*K+2] = true; bd[0][2*K+2] = true; bd[0][2*K+1] = true; bd[0][2*K] = true
	} else {
		bd[3][2*K+2] = true; bd[4][2*K+2] = true; bd[4][2*K+1] = true; bd[4][2*K] = true
	}
	cnt := 0; for i:=0;i<5;i++ { for j:=0;j<2*K+3;j++ { if bd[i][j] { cnt++ } } }
	fmt.Fprintln(wrtr,cnt)
	for i:=0;i<5;i++ { for j:=0;j<2*K+3;j++ { if bd[i][j] { fmt.Fprintf(wrtr,"%v %v\n",i,j) } } }
}
