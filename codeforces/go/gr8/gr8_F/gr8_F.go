...
func main() {
	...
	// PROGRAM STARTS HERE
	// best we can do is to divide circle up into w regions of no more than k-1 on lamps and one off lamp
	// best result will be n - w - (k-1).  Note w = ceil(n/k)
	best := ia(1001)
	bestk := ia(1001)
	for n:=1;n<=1000;n++ {
		for k:=2;2*k <=n; k++ {
			nr := (n+k-1)/k
			cand := n - nr - (k-1)
			if cand > best[n] { best[n],bestk[n] = cand,k}
		}
	}
	N := gi()
	k := bestk[N]
	endgoal := best[N]
	if endgoal == 0 { fmt.Fprintln(wrtr,0); wrtr.Flush(); return }
	ideal := make([]bool,N)
	bd := make([]bool,N)
	for i:=0;i<N;i++ { if i % k == 0 { ideal[i] = false } else { ideal[i] = true } }
	working := []int{}
	for {
		cur := 0; for i:=0;i<N;i++ { if bd[i] { cur++ } }
		if cur >= endgoal {	fmt.Fprintln(wrtr,0); wrtr.Flush(); break }
		working = working[:0]
		for i:=0;i<N && len(working) < k; i++ {
			if !bd[i] && ideal[i] { bd[i] = true; working = append(working,i+1) }
		}
		fmt.Fprintf(wrtr,"%v %v\n",k,vecintstring(working)); wrtr.Flush()
		res := gi(); if res == -1 { panic("Should not get here!") }; res--
		for i:=res;i<res+k;i++ { bd[i%N] = false }
	}
}
