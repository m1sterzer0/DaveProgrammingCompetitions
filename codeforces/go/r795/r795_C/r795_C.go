...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N,K := gi(),gi(); s := gs()
		cnt1 := 0; ans := 0; for _,c := range s { if c == '1' { cnt1++; ans += 11 } }
		if ans != 0 {
			l1pos := N-1; for s[l1pos] == '0' { l1pos-- }
			f1pos := 0; for s[f1pos] == '0' { f1pos++ }
			if cnt1 == 1 {
				// Three cases -- move middle 1 to last, move middle 1 to first, move first 1 to last
				if K >= N-1-l1pos {
					ans -= 10
				} else if K >= l1pos {
					ans -= 1
				...
				if K >= N-1-l1pos { K -= (N-1-l1pos); ans -= 10 }
				if K >= f1pos     { ans -= 1 }
			}
		}
