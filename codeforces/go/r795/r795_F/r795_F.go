...
func main() {
	...
	// PROGRAM STARTS HERE
	N,K := gi(),gi(); X,Y := fill2(N-1); for i:=0;i<N-1;i++ { X[i]--; Y[i]-- }
	fact,factinv := makefact64(int64(N+10),MOD)
	comb := func(n,r int) int64 { if n < 0 || r < 0 || r > n { return 0 }; return fact[n] * factinv[r] % MOD * factinv[n-r] % MOD }
	gr := make([][]int,N)
	for i:=0;i<N-1;i++ { x,y := X[i],Y[i]; gr[x] = append(gr[x],y); gr[y] = append(gr[y],x) }
	var dfs1 func(n,p int) int
	szarr := ia(N)
	dfs1 = func(n,p int) int {
		sz := 1;
		for _,c := range gr[n] { if c == p { continue }; sz += dfs1(c,n) }
		szarr[n] = sz; return sz
	}
	dfs1(0,-1)
	ans := int64(0)
	bsize := ia(N)
	...
	dfs2 = func(n,p int) {
		used := 1; bsize = bsize[:0]
		for _,c := range gr[n] { if c == p { continue }; bsize = append(bsize,szarr[c]); used += szarr[c] }
		if used < N { bsize = append(bsize,N-used) } // these are the nodes on top
		// Now we do when we are root.  We need to count all of the graphs minus the graphs that are contained in exactly one sidebranch
		branchsum := int64(0); for _,x := range bsize { branchsum += comb(x,K) }; branchsum %= MOD
		ans += (comb(N,K) + MOD - branchsum) % MOD * int64(N) % MOD
		// Now we loop through the possibility of the roots in the branches
		for _,x := range bsize {
			rootChoices := int64(x)
			numGraphs := (comb(N-x,K) + MOD - (branchsum - comb(x,K))) % MOD
			subGraphSize := int64(N-x)
			ans += rootChoices * numGraphs % MOD * subGraphSize % MOD
		}
		ans %= MOD
