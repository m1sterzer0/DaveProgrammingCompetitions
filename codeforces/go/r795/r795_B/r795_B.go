...
func main() {
	...
	for tt:=1;tt<=T;tt++ {
		N := gi(); S := gis(N)
		good := true; next := 1; idx := 0; P := ia(N)
		for idx < N {
			s := S[idx]
			idx2 := idx; for idx2+1<N && S[idx2+1] == s { idx2++ }
			if idx2 == idx { good = false; break }
			P[idx2] = next; next++
			for i:=idx;i<idx2;i++ { P[i] = next; next++ }
			idx = idx2+1
		}
		if !good { fmt.Fprintln(wrtr,-1) } else { fmt.Fprintln(wrtr,vecintstring(P)) }
	}
}
