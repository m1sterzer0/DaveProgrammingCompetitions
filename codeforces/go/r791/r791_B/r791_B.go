...
type node struct { sum,cnt int64 }
func lstop(a,b node) node { return node{a.sum+b.sum,a.cnt+b.cnt} }
func lstmap(f int, a node) node { if f == -1 { return a } else { return node{int64(f) * a.cnt,a.cnt}} }
func lstcmp(f int, g int) int { if f == -1 { return g } else { return f } } 
type lazysegtree struct {
	n, size, log int; op func(node, node) node; mapping func(int, node) node
	composition func(int, int) int; e node; id int; d []node; lz []int
}
func Newlazysegtree(n int, op func(node, node) node, mapping func(int, node) node, composition func(int, int) int, e node, id int) *lazysegtree {
	v := make([]node, n); for i := 0; i < n; i++ { v[i] = e }
	return NewlazysegtreeVec(v, op, mapping, composition, e, id)
}
func NewlazysegtreeVec(v []node, op func(node, node) node, mapping func(int, node) node, composition func(int, int) int, e node, id int) *lazysegtree {
...
func main() {
	...
	N,Q := gi(),gi()
	A := gis(N)
	v := make([]node,N); for i:=0;i<N;i++ { v[i] = node{int64(A[i]),1} }
	lst := NewlazysegtreeVec(v,lstop,lstmap,lstcmp,node{0,0},-1)
	for i:=0;i<Q;i++ {
		t := gi()
		if t == 1 {
			i,x := gi(),gi(); lst.Set(i-1,node{int64(x),1})
		} else {
			x := gi(); lst.ApplyRange(0,N-1,x)
		}
		fmt.Fprintln(wrtr,lst.Allprod().sum)
	}
}
