...
func solve(N,Q int, A,P,V []int) string {
	ansarr := ia(0)
	A2 := ia(N)
	sumones := 0
	oddset := Newrbtreeset(func(a,b int) bool { return a < b })
	for i,a := range A {
		A2[i] = bits.OnesCount64(uint64(a))
		sumones += A2[i]
		if A2[i] % 2 == 1 { oddset.Add(i) }
	}
	for i:=0;i<Q;i++ {
		p,v := P[i],V[i]
		newa2 := bits.OnesCount64(uint64(v))
		sumones -= A2[p]; sumones += newa2
		if A2[p] % 2 == 0 && newa2 % 2 == 1 { oddset.Add(p) }
		if A2[p] % 2 == 1 && newa2 % 2 == 0 { oddset.Delete(p) }
		A2[p] = newa2
		if sumones % 2 == 0 { 
