...
func main() {
	...
	solveit := func(a []int, n, k int) []int {
		presum := ia(n+1); presum[0] = 0; for i:=1;i<=n;i++ { presum[i] = presum[i-1] + a[i-1] }
		sufsum := ia(n+1); sufsum[0] = 0; for i:=1;i<=n;i++ { sufsum[i] = sufsum[i-1] + a[n-i] }
		ansarr := ia(n+1)
		for i:=1;i<=n && i<=k; i++ {
			res := 0
			for j:=0;j<=i;j++ {
				res = max(res,presum[j]+sufsum[i-j])
			}
			ansarr[i] = res
		}
		return ansarr
    ...
    for tt:=1;tt<=T;tt++ {
	    // PROGRAM STARTS HERE
		N := gi(); A := gis(N); M := gi(); B := gis(M); K := gi()
		preA := solveit(A,N,K)
		preB := solveit(B,M,K)
		ans := 0
		for i:=0;i<=K;i++ {
