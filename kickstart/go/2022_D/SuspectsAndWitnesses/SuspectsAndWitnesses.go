...
func main() {
    ...
    for tt:=1;tt<=T;tt++ {
	    // PROGRAM STARTS HERE
		N,M,K := gi(),gi(),gi(); A,B := fill2(M); for i:=0;i<M;i++ { A[i]--; B[i]-- }
		gr := make([][]int,N)
		for i:=0;i<M;i++ { a,b := A[i],B[i]; gr[b] = append(gr[b],a) }
		visited := make([]bool,N)
		working := make([]int,0,K+1)
		var dfs func(n int)
		dfs = func(n int) {
			visited[n] = true; working = append(working,n)
			if len(working) > K { return }
			for _,c := range(gr[n]) {
		...
		good := make([]bool,N); for i:=0;i<N;i++ { good[i] = true }
		ans := 0
		for i:=0;i<N;i++ {
			if !good[i] { continue }
			dfs(i)
			if len(working) <= K { for _,w := range working { good[w] = false } } else { ans++ }
			for _,w := range working { visited[w] = false }
			working = working[:0]
		}
