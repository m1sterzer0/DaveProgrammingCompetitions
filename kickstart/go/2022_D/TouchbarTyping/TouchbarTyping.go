...
func main() {
    ...
    T := gi()
	var dp [2500][2501][2]int
	for tt:=1;tt<=T;tt++ {
	    // PROGRAM STARTS HERE
		N := gi(); S := gis(N); M := gi(); K := gis(M)
		for i:=0;i<M;i++ { for j:=0;j<=2500;j++ { for k:=0;k<2;k++ { dp[i][j][k] = inf } } }
		for d := M-1; d>=0; d-- {
			for i:=0;i+d<M;i++ { j := i+d; dp[i][K[j]][1] = d; dp[j][K[i]][0] = d }
		}
		d1 := iai(M,inf); d2 := iai(M,inf)
		for i:=0;i<M;i++ { if K[i] == S[0] { d1[i] = 0 } }
		for i:=1;i<N;i++ {
			c := S[i]
			for j:=0;j<M;j++ { d2[j] = inf }
			for j:=0;j<M;j++ {
				if d1[j] == inf { continue }
				dd := dp[j][c][0]; if dd != inf { d2[j-dd] = min(d2[j-dd],d1[j]+dd) }
				dd  = dp[j][c][1]; if dd != inf { d2[j+dd] = min(d2[j+dd],d1[j]+dd) }
			}
			d1,d2 = d2,d1
		}
		ans := minarr(d1)
