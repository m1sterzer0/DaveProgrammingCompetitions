...
func main() {
    ...
    T := gi()
    for tt:=1;tt<=T;tt++ {
		N,O,D := gi(),gi(),gi(); X1,X2,A,B,C,M,L := gi(),gi(),gi(),gi(),gi(),gi(),gi()
		X,S := ia(N),ia(N); X[0],X[1] = X1,X2; S[0],S[1] = (X1+L),(X2+L)
		for i:=2;i<N;i++ { X[i] = (A * X[i-1] + B * X[i-2] + C) % M; S[i] = X[i] + L }
		cumsumarr := ia(N); ss := 0; for i,s := range S { ss += s; cumsumarr[i] = ss }
		lf,rt,numodd := -1,-2,0
		myset := Newrbtreemultiset(func (a,b int) bool { return a < b })
		best := -inf
		for i:=0;i<N;i++ {
			if myset.Len() == 0 {
				lf,rt,numodd = i,i-1,0
			} else {
				if S[lf] & 1 == 1 { numodd-- }
				myset.Delete(cumsumarr[lf])
				lf++
			}
			for rt+1 < N && numodd + (S[rt+1] & 1) <= O {
				rt++
				myset.Add(cumsumarr[rt])
				numodd += S[rt] & 1
			}
			prevsum := 0
			if i > 0 { prevsum = cumsumarr[i-1] }
			if myset.Len() > 0 && myset.MinKey() <= D + prevsum {
				k2,_ := myset.FindLe(D+prevsum)
				cand := k2 - prevsum
				best = max(best,cand)
			}
			//fmt.Printf("DBG: tt:%v i:%v S[i]:%v best:%v\n",tt,i,S[i],best)
		}
		if best == -inf {
