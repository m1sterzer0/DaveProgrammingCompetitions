...
func main() {
	...
	// PROGRAM STARTS HERE
	N,M := gi2(); A,B,C := fill3(M)
	E := make([]edge,M); for i:=0;i<M;i++ { E[i] = edge{A[i],B[i],C[i]} }
	sort.Slice(E,func(i,j int) bool { return E[i].c < E[j].c } )
	uf := NewDsu(N+1)
	keepsum := 0
	for _,e := range E { if e.c <= 0 || uf.Leader(e.a) != uf.Leader(e.b) { keepsum += e.c; uf.Merge(e.a,e.b) } }
	ans := sumarr(C) - keepsum
	fmt.Println(ans)
}
