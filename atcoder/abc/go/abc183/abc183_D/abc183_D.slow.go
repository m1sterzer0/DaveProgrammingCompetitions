...
func stsum(a,b int) int { return a+b }
func main() {
	...
	N,W := gi2(); S,T,P := fill3(N)
	st := Newlazysegtree(200001,max,stsum,stsum,0,0)
	for i:=0;i<N;i++ { st.ApplyRange(S[i],T[i]-1,P[i]) }
	demand := st.Allprod()
	ans := "Yes"; if demand > W { ans = "No" }; fmt.Println(ans)
}
