...
func main() {
	...
	N,Q := gi2(); C := gis(N); T,A,B := fill3(Q)
	cc := make([]map[int]int,N+1); for i:=0;i<N+1;i++ { cc[i] = make(map[int]int) }
	uf := NewDsu(N+1)
	for i,c := range C { cc[i+1][c] = 1 }
	for i:=0;i<Q;i++ {
		if T[i] == 1 {
			l1 := uf.Leader(A[i]); l2 := uf.Leader(B[i])
			if l1 == l2 { continue }
			uf.Merge(l1,l2); l3 := uf.Leader(l1)
			cmap,pmap := cc[l1],cc[l2]; if l1 == l3 { cmap,pmap = pmap,cmap }
			for k,v := range cmap { pmap[k] += v }
		} else {
			l1 := uf.Leader(A[i])
			fmt.Fprintln(wrtr,cc[l1][B[i]])
		}
	}
}
